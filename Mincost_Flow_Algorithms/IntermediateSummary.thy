...
definition "invar_aux18 state \<equiv> adj_inv (\<FF>_imp state)"
...
definition "goodF F \<equiv> ((\<forall> v . \<exists> neighb . (lookup F v = Some neighb))
...
definition "insert_undirected_edge u v forst \<equiv> (let neighbs_u = the (lookup forst u);

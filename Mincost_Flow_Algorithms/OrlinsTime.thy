...
theory OrlinsTime
  imports LoopATime LoopBTime Orlins LaminarFamily
begin

locale orlinsTime =
loopATime where fst = fst + 
loopBTime where fst = fst + 
orlins where fst = fst 
for fst::"'edge_type \<Rightarrow> 'a"+
fixes t\<^sub>O\<^sub>C::nat and  t\<^sub>O\<^sub>B::nat
begin 

function (domintros) orlinsTime::"nat \<Rightarrow> ('a, 'd, 'c, 'edge_type) Algo_state  
                         \<Rightarrow> nat \<times> ('a, 'd, 'c, 'edge_type) Algo_state" where
"(orlinsTime tt\<^sub>O\<^sub>C  state) = (if (return state = success) then (tt\<^sub>O\<^sub>C, state)
...
lemma loopA_dom_aux_invar: "aux_invar state \<Longrightarrow> loopA_dom state"
  using aux_invar_def termination_of_loopA by blast
...
lemma invar_gamma_pres_orlins_one_step_time:
  ...
  subgoal loopB_dom
    ...
    by(fastforce intro!: loopA_dom_aux_invar loopB_termination loopA_invar_gamma_pres aux_invar_gamma 
       gamma_upd_aux_invar_pres[of state] invar_aux17_from_aux_invar | simp)+
  subgoal
    by(intro loopB_invar_gamma_pres[OF  loopB_dom] | 
       subst sym[OF equal_results_A] | 
       fastforce intro!: loopA_dom_aux_invar  loopA_invar_gamma_pres aux_invar_gamma 
       gamma_upd_aux_invar_pres[of state] invar_aux17_from_aux_invar| simp)+
  done
...
lemma orlins_time_and_not_time_one_step_check_equal:
  ...
  by (subst sym[OF equal_results_A]  sym[OF equal_results_B] | 
     fastforce intro!: loopB_termination loopA_invar_gamma_pres arg_cong[of _ _ loopB] 
                      sym[OF equal_results_A] loopA_dom_aux_invar aux_invar_gamma invar_aux17_from_aux_invar)+
...
next
  ...
  define state'time where "state'time = loopAtime (already_done\<lparr>current_\<gamma> := \<gamma>'\<rparr>)"
  define state''time where "state''time = loopBtime (prod.snd state'time)"
  ...
  define timeA where "timeA = (card (comps \<V> (to_graph (\<FF>_imp already_done))) - 
      card (comps \<V> (to_graph (\<FF>_imp (prod.snd (loopAtime 
  ...
  have timeA:"prod.fst state'time  = timeA"
    using aux_invar_after_gamma 
          loopA_dom_aux_invar[simplified ] 
  ...
  have aux_invar_state'time:"aux_invar (prod.snd state'time)"
    ...
    by(subst  sym[OF equal_results_A], 
       auto intro: loopA_dom_aux_invar loopA_invar_aux_pres invar_aux17_from_aux_invar)
  ...
  have invar_gamma_state'time:"invar_gamma (prod.snd state'time)"
    using  sym[OF equal_results_A[OF _ invar_aux17_from_aux_invar]]  aux_invar_after_gamma  gamma_upd_aux_invar_pres[of already_done,
           ...
           sym[OF \<gamma>'_def[ simplified E'_def f_def  \<gamma>_def b_def sym[OF new_\<gamma>_def[simplified Let_def]]]]]
           i_notyetterm orlins_compow_time_invars_pres[OF Suc(4) Suc(5) Suc(6) refl] 
    by (fastforce intro:  loopA_dom_aux_invar loopA_invar_gamma_pres simp add: already_done_def state'time_def)
  have loopB_time_phi:"prod.fst (loopBtime (prod.snd state'time))
  ...
  have Phi_mod_A:"\<Phi>  (prod.snd state'time)
       \<le> \<Phi> (already_done\<lparr>current_\<gamma> := \<gamma>'\<rparr>) + int (card (comps \<V> (to_graph (\<FF>_imp already_done)))) -
             int (card (comps \<V> (to_graph (\<FF>_imp (loopA (already_done\<lparr>current_\<gamma> := \<gamma>'\<rparr>))))))"
    using equal_results_A[OF loopA_dom_aux_invar invar_aux17_from_aux_invar] 
  ...
  have "to_graph (\<FF>_imp (prod.snd state'time)) = to_graph (\<FF>_imp (prod.snd state''time))"
    unfolding state''time_def
    apply(subst sym[OF equal_results_B])
    using  sym[OF loopB_changes_\<FF>_imp] loopB_termination invar_gamma_state'time by auto
  ...
  have card_loopA_reduce:"card (comps \<V> (to_graph (\<FF>_imp already_done))) \<ge>
             card (comps \<V> (to_graph (\<FF>_imp (loopA (already_done\<lparr>current_\<gamma> := \<gamma>'\<rparr>)))))"
    using card_decrease aux_invar_after_gamma loopA_dom_aux_invar by force
  hence "\<Phi> (already_done\<lparr>current_\<gamma> := \<gamma>'\<rparr>) + int (card (comps \<V> (to_graph (\<FF>_imp already_done)))) -
         ...
         \<Phi> (already_done\<lparr>current_\<gamma> := \<gamma>'\<rparr>) +  (card (comps \<V> (to_graph (\<FF>_imp already_done)))) -
             card (comps \<V> (to_graph (\<FF>_imp (loopA (already_done\<lparr>current_\<gamma> := \<gamma>'\<rparr>)))))" by auto
  ...
  have Phi_number_important:"state' = (orlins_one_step_check ^^ i) state \<Longrightarrow>
   ...
   also have "... \<le> (card (comps \<V> (to_graph (\<FF>_imp already_done))) 
     ...
     using sym[OF equal_results_A[OF loopA_dom_aux_invar[OF aux_invar_after_gamma]]]  
           add_mult_distrib2 less_or_eq_imp_le 
     by (auto simp add: add_mult_distrib diff_add_assoc[OF card_loopA_reduce] 
   ...
   have forest_final:"to_graph (\<FF>_imp (local.loopA (already_done\<lparr>current_\<gamma> := \<gamma>'\<rparr>))) 
                         = to_graph (\<FF>_imp (prod.snd final))" 
     unfolding Suc(2)
     apply(subst funpow.simps(2), subst o_apply, subst (2) surjective_pairing, subst orlins_one_step_time_check.simps)
     apply(subst if_not_P, simp add: i_notyetterm)+
     using  loopA_invar_gamma_pres aux_invar_after_gamma invar_gamma_after_gamma 
             sym[OF equal_results_A] sym[OF equal_results_B] 
              loopB_termination loopA_dom_aux_invar sym[OF loopB_changes_\<FF>_imp] 
                       loopA_invar_gamma_pres invar_aux17_from_aux_invar 
  ...
  show ?case 
    ...
    using card_loopA_reduce card_decrease_done 
...
proof-
  ...
  have aux_invar_after_gamma: "aux_invar (already_done\<lparr>current_\<gamma> := \<gamma>'\<rparr>)"
    using assms(1-6) already_done_def aux_invar_gamma 
          orlins_compow_time_invars_pres by blast
  define timeA where "timeA = (card (comps \<V> (to_graph (\<FF>_imp already_done)))
                   - card (comps \<V> (to_graph (\<FF>_imp (prod.snd (loopAtime (already_done\<lparr>current_\<gamma> := \<gamma>'\<rparr>))))))) *
  (t\<^sub>A\<^sub>u\<^sub>f + t\<^sub>A\<^sub>C + t\<^sub>A\<^sub>B) +
  (t\<^sub>A\<^sub>u\<^sub>f + t\<^sub>A\<^sub>C)"
  have aux_invar_after_gamma: "aux_invar (already_done\<lparr>current_\<gamma> := \<gamma>'\<rparr>)"
  ...
  have invar_gamma_state'time:"invar_gamma (prod.snd state'time)"
    using  sym[OF equal_results_A]  aux_invar_after_gamma  gamma_upd_aux_invar_pres[of already_done,
           ...
           sym[OF \<gamma>'_def[ simplified E'_def f_def  \<gamma>_def b_def sym[OF new_\<gamma>_def[simplified Let_def]]]]]
           i_notyetterm orlins_compow_time_invars_pres[OF assms(3,4,5) refl] 
    by (fastforce intro!:  loopA_dom_aux_invar loopA_invar_gamma_pres invar_aux17_from_aux_invar simp add: already_done_def state'time_def)
  have loopB_time_phi:"prod.fst (loopBtime (prod.snd state'time))
       \<le> nat (\<Phi> (prod.snd state'time)) * (t\<^sub>B\<^sub>C + t\<^sub>B\<^sub>B + t\<^sub>B\<^sub>u\<^sub>f) + (t\<^sub>B\<^sub>F + t\<^sub>B\<^sub>C + t\<^sub>B\<^sub>u\<^sub>f)"
    using  time_boundB[OF invar_gamma_state'time refl] by simp
  have Phi_mod_A:"\<Phi>  (prod.snd state'time)
       \<le> \<Phi> (already_done\<lparr>current_\<gamma> := \<gamma>'\<rparr>) + int (card (comps \<V> (to_graph (\<FF>_imp already_done)))) -
             int (card (comps \<V> (to_graph (\<FF>_imp (loopA (already_done\<lparr>current_\<gamma> := \<gamma>'\<rparr>))))))"
    using equal_results_A[OF loopA_dom_aux_invar] aux_invar_after_gamma          
      Phi_increase invar_gamma_after_gamma
      invar_aux17_from_aux_invar 
    by (force simp add: state'time_def)
  have "to_graph (\<FF>_imp (prod.snd state'time)) = to_graph (\<FF>_imp (prod.snd state''time))"
    using  sym[OF equal_results_B]  sym[OF loopB_changes_\<FF>_imp] loopB_termination invar_gamma_state'time
    by (simp add:  state''time_def)
  have "to_graph (\<FF>_imp (prod.snd final)) = to_graph (\<FF>_imp (prod.snd state''time))"
    unfolding Suc 
    apply(subst funpow.simps(2), subst o_apply, subst (2) surjective_pairing, subst orlins_one_step_time_check.simps)
  ...
  have Phi_number_important:"state' = (orlins_one_step_check ^^ i) state \<Longrightarrow>
   ...
   also have "... \<le> timeA + nat (\<Phi> (prod.snd state'time)) * (t\<^sub>B\<^sub>C + t\<^sub>B\<^sub>B + t\<^sub>B\<^sub>u\<^sub>f)
     ...
     using timeA loopB_time_phi by(simp add: state''time_def)
   also have "... \<le> timeA + ( nat(\<Phi> (already_done\<lparr>current_\<gamma> := \<gamma>'\<rparr>)) + 
                             (card (comps \<V> (to_graph (\<FF>_imp already_done)))) -
                               card (comps \<V> (to_graph (\<FF>_imp (local.loopA (already_done\<lparr>current_\<gamma> := \<gamma>'\<rparr>)))))) * 
                            (t\<^sub>B\<^sub>C + t\<^sub>B\<^sub>B + t\<^sub>B\<^sub>u\<^sub>f) + (t\<^sub>B\<^sub>F + t\<^sub>B\<^sub>C + t\<^sub>B\<^sub>u\<^sub>f) +
...
theorem running_time_sum_from_init:
  assumes 
          "final = (orlins_one_step_time_check  ^^ i) (loopBtime initial)"
          "return (prod.snd final) \<noteq> notyetterm"
          "\<And> j final. j < i \<Longrightarrow> final = (orlins_one_step_time_check  ^^ j)  (loopBtime initial)
              \<Longrightarrow> return (prod.snd final) = notyetterm"
        ...
        shows "prod.fst final \<le> (card (comps \<V> (to_graph (\<FF>_imp initial))) 
                            ...
                            (t\<^sub>A\<^sub>u\<^sub>f + t\<^sub>A\<^sub>C + t\<^sub>A\<^sub>B + t\<^sub>B\<^sub>C + t\<^sub>B\<^sub>B + t\<^sub>B\<^sub>u\<^sub>f) 
                   +  i * (t\<^sub>B\<^sub>F + t\<^sub>B\<^sub>C + t\<^sub>B\<^sub>u\<^sub>f + t\<^sub>A\<^sub>u\<^sub>f + t\<^sub>A\<^sub>C + t\<^sub>O\<^sub>C + t\<^sub>O\<^sub>B ) +
                     ((\<Sum> j \<in> {1..i}. (let state' = ((orlins_one_step_check ^^ (j - 1))  (loopB initial)) in 
                                                     card { v. important state' v} )) +
                                                     card { v. important_initial initial v})
                           *(t\<^sub>B\<^sub>C + t\<^sub>B\<^sub>B + t\<^sub>B\<^sub>u\<^sub>f)+
                       (t\<^sub>B\<^sub>F + t\<^sub>B\<^sub>C + t\<^sub>B\<^sub>u\<^sub>f )"
proof-
  have "prod.fst (loopBtime initial) \<le> nat (\<Phi> initial) * (t\<^sub>B\<^sub>C + t\<^sub>B\<^sub>B + t\<^sub>B\<^sub>u\<^sub>f) + (t\<^sub>B\<^sub>F + t\<^sub>B\<^sub>C + t\<^sub>B\<^sub>u\<^sub>f)"
  ...
  have "prod.fst (loopBtime initial) \<le> nat (\<Phi> initial) * (t\<^sub>B\<^sub>C + t\<^sub>B\<^sub>B + t\<^sub>B\<^sub>u\<^sub>f) + (t\<^sub>B\<^sub>F + t\<^sub>B\<^sub>C + t\<^sub>B\<^sub>u\<^sub>f)"
    using time_boundB[OF invar_gamma_initial[OF assms(4)] refl] by simp
  have same_F_After_loopB: "to_graph (\<FF>_imp (prod.snd (loopBtime initial))) = to_graph (\<FF>_imp initial)"
    using equal_results_B loopB_termination[OF invar_gamma_initial[OF assms(4)]]
          loopB_changes_\<FF>_imp by simp
  have same_F_After_loopB': "to_graph (\<FF>_imp (loopB initial)) = to_graph (\<FF>_imp initial)"
    using equal_results_B loopB_termination[OF invar_gamma_initial[OF assms(4)]]
          loopB_changes_\<FF>_imp by simp
  have take_first_time_out:"((prod.fst (loopBtime initial)) +++ ((orlins_one_step_time_check ^^ i) (0, loopB initial))) =
        (orlins_one_step_time_check ^^ i) (loopBtime initial)" for i
    using runtime_add_constant[of i "prod.fst (loopBtime initial)" 0 "loopB initial"] 
          equal_results_B loopB_termination[OF invar_gamma_initial[OF assms(4)]] 
    by(simp add: add_fst_def)
  have loopBtime_bound:"prod.fst (loopBtime initial) \<le>
...
proof(cases i)
  case 0
  then show ?thesis
    using assms 0 same_F_After_loopB Phi_number_important loopBtime_bound
    by simp
next
  case (Suc nat)
  have aux_invar_loopB:"aux_invar (loopB initial)"
    using assms(4) aux_invar_initial invar_gamma_initial loopB_termination loopB_axioms
          loopB_invar_aux_pres by blast
  have invar_gamma_loopB:"invar_gamma (loopB initial)"
    using assms(4) invar_gamma_initial loopB_invar_gamma_pres loopB_termination by blast
  have invar_non_zero_b_loopB:"invar_non_zero_b (loopB initial)"
    using assms(3)[of 0, OF _ refl, simplified] Suc  equal_results_B 
          loopB_termination[OF invar_gamma_initial[OF assms(4)]] 
    by (fastforce intro!: remaining_balance_after_loopB[OF _ refl])  
  have orlins_entry_loopB:"orlins_entry (local.loopB initial)"
    using assms(3)[of 0, OF _ refl, simplified] Suc  equal_results_B 
          loopB_termination invar_gamma_initial[OF assms(4)] 
    by (fastforce intro: orlins_entry_after_loopB[OF _ refl])
  have sum_bound:"prod.fst ((orlins_one_step_time_check ^^ Suc nat) (0, local.loopB initial))
  \<le> (card (comps \<V> (to_graph (\<FF>_imp (local.loopB initial)))) -
      card (comps \<V> (to_graph (\<FF>_imp (prod.snd ((orlins_one_step_time_check ^^ Suc nat) (0, local.loopB initial))))))) *
     (t\<^sub>A\<^sub>u\<^sub>f + t\<^sub>A\<^sub>C + t\<^sub>A\<^sub>B + t\<^sub>B\<^sub>C + t\<^sub>B\<^sub>B + t\<^sub>B\<^sub>u\<^sub>f) +
     nat * (t\<^sub>B\<^sub>F + t\<^sub>B\<^sub>C + t\<^sub>B\<^sub>u\<^sub>f + t\<^sub>A\<^sub>u\<^sub>f + t\<^sub>A\<^sub>C + t\<^sub>O\<^sub>C + t\<^sub>O\<^sub>B) +
     (\<Sum>j = 1..Suc nat.
         let state' = (orlins_one_step_check ^^ (j - 1)) (local.loopB initial) in card {v. important state' v}) *
     (t\<^sub>B\<^sub>C + t\<^sub>B\<^sub>B + t\<^sub>B\<^sub>u\<^sub>f) +
     (t\<^sub>B\<^sub>F + t\<^sub>B\<^sub>C + t\<^sub>B\<^sub>u\<^sub>f + t\<^sub>A\<^sub>u\<^sub>f + t\<^sub>A\<^sub>C + t\<^sub>O\<^sub>C + t\<^sub>O\<^sub>B)"
  apply(rule running_time_sum[OF refl, of nat "loopB initial", 
      OF _  aux_invar_loopB invar_gamma_loopB invar_non_zero_b_loopB orlins_entry_loopB])
    ...
    by fastforce+
  have take_out:"prod.fst ((orlins_one_step_time_check ^^ i) (loopBtime initial)) = 
       prod.fst (loopBtime initial) + prod.fst ((orlins_one_step_time_check ^^ i) (0, local.loopB initial))"
    apply(subst (2) surjective_pairing,
          subst sym[OF equal_results_B[OF loopB_termination[OF invar_gamma_initial[OF assms(4)]]]])
    using runtime_add_constant[of i "prod.fst (loopBtime initial)" 0, simplified add_fst_def]
    by simp
  show ?thesis 
    apply(subst assms(1))+ (*REWRITE*)
    apply(subst take_out)  
    apply(subst (5) surjective_pairing)
    apply(subst sym[OF equal_results_B[OF loopB_termination[OF invar_gamma_initial[OF assms(4)]]]])
    apply(subst runtime_add_constant[of i "prod.fst (loopBtime initial)" 0, simplified add_fst_def, simplified])
    apply(subst snd_conv) (*ALGEBRA*)
    apply(rule order.trans)
    apply(rule add_mono[OF  loopBtime_bound sum_bound[simplified sym[OF Suc]]])
    using  same_F_After_loopB' Phi_number_important Suc
    by (auto simp add: algebra_simps)
 qed
qed
...
lemma number_of_important:
  assumes "final = (orlins_one_step_check  ^^ i) (loopB initial)"
          "return final \<noteq> notyetterm"
          "\<And> j final. j < i \<Longrightarrow> final = (orlins_one_step_check  ^^ j)  (loopB initial)
              \<Longrightarrow> return final = notyetterm"
          "\<not> (\<forall> v \<in> \<V>. \<b> v = 0)"
    shows "((\<Sum> j \<in> {1..i}. (let state' = ((orlins_one_step_check ^^ (j - 1))  (loopB initial)) in 
                                                     card { v. important state' v} )) +
                                                     card { v. important_initial initial v})
          \<le> (\<l> + 1) * (2*N - 1)"
proof- 
  ...
  have loopB_sim:"orlins_one_step_check pseudo_initial = loopB initial"
    ...
    by(subst loopA_simps(2), intro loopA_ret_condI )
      (fastforce intro!: loopA_ret_condI 
               simp add: loopA_simps(2) mult_less_0_iff)+
  ...
  have invar_forest_pres: "j \<le> i \<Longrightarrow> invar_forest ((orlins_one_step_check ^^ j) pseudo_initial)" for j
    ...
    using invar_forest_after_one assms(4) aux_invar_initial invar_gamma_initial loopB_invar_aux_pres
          loopB_sim loopB_termination loopB_invar_gamma_pres invar_non_zero_b_pres[of 1, simplified]
  ...
  have invar_integral_pres: "j \<le> i \<Longrightarrow> invar_integral ((orlins_one_step_check ^^ j) pseudo_initial)" for j
    ...
    apply(simp, subst Suc_pred', simp, subst funpow_Suc_right, simp)
    using aux_invar_initial  loopB_invar_aux_pres  assms(4) invar_gamma_initial loopB_invar_gamma_pres
          loopB_termination assms(3) loopB_sim loopB_term remaining_balance_after_loopB 
  ...
  have orlins_entry_pres: "j \<le> i \<Longrightarrow> orlins_entry ((orlins_one_step_check ^^ j) pseudo_initial)" for j
    ...
    using loopB_sim assms(3)[of "j-1", OF _ refl]  orlins_entry_pseudo_initial 
    by  auto
    ...
    next
      ...
      have "return state \<noteq> notyetterm \<Longrightarrow> False"
        using pseudo_initial_def initial_def  ii_leq_i 1 j_leq_i
              assms(3)[of "ii-1", OF _ refl, simplified sym[OF loopB_sim] 
      ...
      have "connected_component (to_graph (\<FF>_imp state)) v 
        ...
        using   1(1) k_prop(1) unfolding sym[OF loopB_sim] 
  ...
  have "((\<Sum> j \<in> {1..i}. (let state' = ((orlins_one_step_check ^^ (j - 1))  (loopB initial)) in 
                                                     card { v. important state' v} )) +
                                                     card { v. important_initial initial v}) =
...
lemma orlinsTime_dom_base_shift: "orlinsTime_dom (t, state) \<Longrightarrow> orlinsTime_dom (s, state)"
proof(induction  arbitrary: s rule: orlinsTime.pinduct)
  case (1 t state)
  note IH = this
  show ?case
  proof(rule orlinsTime.domintros, goal_cases)
    case (1 aa ba ab bb)
    show ?case 
      apply(rule IH(2))
      using 1 by auto
...
lemma orlinsTime_dom_base_extract: 
"orlinsTime_dom (t + s, state) \<Longrightarrow> 
(orlinsTime (t + s) state) = (t +++ (orlinsTime s state))"
proof(induction  arbitrary: s t rule: orlinsTime.pinduct)
  case (1 t state)
  note IH = this
  show ?case
    apply(subst orlinsTime.psimps)
    using orlinsTime_dom_base_shift IH(1) apply simp
    apply(subst (2) orlinsTime.psimps)
    using orlinsTime_dom_base_shift IH(1) 
...
"compow (Suc k) orlins_one_step_time_check (tt, state) = (tfin, state') \<Longrightarrow>
       return state' = success \<or> return state' = failure \<Longrightarrow> 
         orlinsTime_dom (tt, state)"
proof(induction k arbitrary: state tt)
  case 0
  have A: "orlinsTime_dom (tt, state)"
  proof(rule orlinsTime.domintros, goal_cases)
    case (1 a b aa ba)
    have "(prod.fst (loopAtime
    ...
    thus ?case 
      using 0(2)
      by(auto intro: orlinsTime.domintros simp add: add_fst_def)
  next
    case (2 a b aa ba e)
    have "(prod.fst (loopAtime
  ...
  case (Suc k)
    ...
    proof(cases "return (prod.snd (orlins_one_step_time_check (tt, state)))")
      ...
      proof(rule orlinsTime.domintros, goal_cases)
        ...
        then show ?case 
           using success(1) 
           by(auto intro: orlinsTime.domintros simp add: orlins_one_step_time_def Let_def orlins_one_step_time_check.simps add_fst_def)
      next
        case (2 a b aa ba e)
        then show ?case
          using 2 success
          unfolding orlins_one_step_time_def Let_def orlins_one_step_time_check.simps add_fst_def
          by(subst (asm) if_not_P, auto intro: orlinsTime.domintros)       
      qed
    ...
    next
      ...
      hence 11:"(orlins_one_step_time_check ^^  k) 
        ...
        by(auto simp add: add_fst_def) 
      have 12:"orlinsTime_dom (tt + prod.fst (orlins_one_step_time  state) , prod.snd (orlins_one_step_time state))"                             
        apply(rule  Suc(1))
        apply(subst funpow_Suc_right)
      ...
      proof(rule orlinsTime.domintros, goal_cases)
        case (1 aa ba ab bb)
        then show ?case 
          using orlinsTime_dom_base_shift[OF 12(1)] 
          by(simp add: orlins_one_step_time_def Let_def)
      next
        case (2 aa ba ab bb e)
        hence snd_same:"prod.snd (orlins_one_step_time state) =
              prod.snd (loopBtime (prod.snd (loopAtime (state\<lparr>current_\<gamma> := current_\<gamma> state / 2\<rparr>))))"
          by(auto simp add: orlins_one_step_time_def Let_def)
        show ?case 
          using sym[OF snd_same]  orlinsTime_dom_base_shift[OF 12(1)] 
          by auto
      qed
      then show ?thesis by simp
    qed
...
lemma succ_fail_term_same_with_time:
  assumes "compow (Suc k) orlins_one_step_time_check (tt, state) = (tfin, state')"
    ...
    shows "orlinsTime tt state  = (tfin, state')"
  using assms
proof(induction arbitrary:  tt k rule:
 orlinsTime.pinduct[OF succ_fail_term_same_with_time_dom, of k tt state tfin state'])
  case 1
  show ?case
  using assms by simp
next
...
next
  ...
  proof(insert 3(1,3,4), 
        subst orlinsTime.psimps, (rule succ_fail_term_same_with_time_dom;auto), 
    ...
    proof(insert 3, subst if_not_P, goal_cases)
      ...
      thus ?case
        ...
        show ?case
          apply(insert 1)
         by((subst add_fst_def Let_def)+,(subst orlinsTime.psimps), 
              (subst add_fst_def Let_def | subst (asm) add_fst_def Let_def )+,
              rule orlinsTime.domintros[of _ tt], simp, simp,
               (subst (2) orlinsTime.psimps),
               ((subst add_fst_def Let_def | subst (asm) add_fst_def Let_def )+,
                 rule orlinsTime.domintros[of _ tt], simp, simp), 
      ...
      next
        case (2 nat)
        show ?case 
        apply(insert 2, (subst Let_def)+, subst sym[OF orlinsTime_dom_base_extract])         
...
theorem running_time_initial:
  assumes "final = orlinsTime t\<^sub>O\<^sub>C (loopB initial)"
  shows "prod.fst final + prod.fst (loopBtime initial) \<le> 
          ...
          +   (t\<^sub>B\<^sub>F + t\<^sub>B\<^sub>C + t\<^sub>B\<^sub>u\<^sub>f ) + t\<^sub>O\<^sub>C 
                         ...
                         \<and> prod.snd final = orlins (loopB initial)"
proof(cases "\<forall> v \<in> \<V>. \<b> v = 0")
  case True
  have a:"prod.fst (loopBtime initial) = (t\<^sub>B\<^sub>F + t\<^sub>B\<^sub>C + t\<^sub>B\<^sub>u\<^sub>f )"
    using True by (subst loopBtime.psimps)(auto intro: loopBtime.domintros simp add: add_fst_def initial_def)
  have bb:"return (loopB initial) = success"
    using True by (subst loopB.psimps)(auto intro: loopB.domintros simp add: add_fst_def initial_def)
  hence "prod.fst final = t\<^sub>O\<^sub>C"
    by (subst assms(1), subst orlinsTime.psimps)(auto intro: orlinsTime.domintros)
  moreover have "orlins (loopB initial) = prod.snd final"
    using bb 
    by (subst orlinsTime.psimps orlins.psimps |
        auto intro: orlins.domintros orlinsTime.domintros simp add: assms(1))+
...
next
  ...
  have loopB_sim:"orlins_one_step_check pseudo_initial = loopB initial"
    ...
    using Max_b_gtr_0 N_def cardV_0 set_get(1-3)[OF invar_filter[OF \<E>_impl_meaning(2)]]
    by(subst loopA_simps(2), intro loopA_ret_condI)
      (fastforce simp add: loopA_simps(2) mult_less_0_iff)+
  have pseudo_initial_important:
       "important pseudo_initial v \<longleftrightarrow> important_initial initial v" for v
    by(simp add: pseudo_initial_def important_def important_initial_def initial_def new_\<gamma>_def)
  have aux_invar_pseudo_initial: "aux_invar pseudo_initial"
  ...
  hence termy:"return ((orlins_one_step_check ^^ (N * (\<l> + \<k> + 2) - 1)) (loopB initial)) \<noteq> notyetterm"
    apply(subst sym[OF loopB_sim])
    apply(subst  sym[OF o_apply[of "_ ^^ _" "orlins_one_step_check"]])
    apply(subst sym[OF funpow_Suc_right])
    using N_gtr_0 by simp
  obtain I where I_prop:"return ((orlins_one_step_check ^^ I) (loopB initial)) \<noteq> notyetterm "
          "\<not> (\<exists> J < I. return ((orlins_one_step_check ^^ J) (loopB initial)) \<noteq> notyetterm)"
    by(rule get_least, rule termy, simp) 
  have switch_timed_and_untimed:"(prod.snd ((orlins_one_step_time_check ^^ Ii) (t\<^sub>O\<^sub>C, loopB initial))) =
         (orlins_one_step_check ^^ Ii)  (loopB initial)" for Ii
    apply(cases I)
    subgoal
      apply(subst notyetterm_no_change)
      using I_prop(1) apply simp
      apply(subst (2) sym[OF snd_conv[of t\<^sub>O\<^sub>C "loopB _"]], subst succ_fail_not_changes_time)
      using I_prop(1)  return.exhaust by auto
      using loopB_invar_aux_pres[OF loopB_termination] aux_invar_initial
            loopB_invar_gamma_pres[OF loopB_termination]  invar_gamma_initial[OF False]  I_prop(2) 
      by (auto intro!: same_as_without_time remaining_balance_after_loopB[OF loopB_termination refl])
    have I_bound: " I \<le> (N * (\<l> + \<k> + 2) - 1)"
      using I_prop termy  not_less by blast
  have I_prop': "return (prod.snd ((orlins_one_step_time_check ^^ I) (t\<^sub>O\<^sub>C, loopB initial))) \<noteq> notyetterm "
          "\<not> (\<exists> J < I. return (prod.snd ((orlins_one_step_time_check ^^ J) (t\<^sub>O\<^sub>C, loopB initial))) \<noteq> notyetterm)"
    using switch_timed_and_untimed I_prop by auto
  have I_prop'': "return (prod.snd ((orlins_one_step_time_check ^^ I) (loopBtime initial))) \<noteq> notyetterm "
          "\<not> (\<exists> J < I. return (prod.snd ((orlins_one_step_time_check ^^ J) (loopBtime initial))) \<noteq> notyetterm)"
    using I_prop' equal_results_B[OF loopB_termination[OF invar_gamma_initial[OF False]]] 
          runtime_add_constant[of _ t\<^sub>O\<^sub>C 0 "loopB initial"] 
          runtime_add_constant[of _ "prod.fst (loopBtime initial)" 0 "loopB initial"]
    by(auto simp add: add_fst_def)
  have to_time:"(orlins_one_step_time_check ^^ I) (t\<^sub>O\<^sub>C, loopB initial) = 
        orlinsTime t\<^sub>O\<^sub>C (loopB initial)"
    apply(cases I)
    subgoal
      using I_prop' return.exhaust 
      by (subst orlinsTime.psimps | auto intro: orlinsTime.domintros)+   
    using  I_prop'(1) return.exhaust 
    by (subst  succ_fail_term_same_with_time[of _ t\<^sub>O\<^sub>C "loopB initial"] | 
                   auto intro: sym[OF prod.collapse])+
  have to_usual: "(orlins_one_step_check ^^ I)  (loopB initial) =
                  orlins (loopB initial)"
    apply(cases I)
    subgoal
      using I_prop return.exhaust 
      by (subst orlins.psimps | auto intro: orlins.domintros)+   
    using  I_prop(1) return.exhaust 
    by (subst  succ_fail_term_same[of _ "loopB initial"] | 
                   auto intro: sym[OF prod.collapse])+
  have time_swap:"prod.fst (loopBtime initial) +++ (orlins_one_step_time_check ^^ I) (t\<^sub>O\<^sub>C, loopB initial) =
       t\<^sub>O\<^sub>C +++  (orlins_one_step_time_check ^^ I) (loopBtime initial)"
    ...
    using equal_results_B[OF loopB_termination[OF invar_gamma_initial[OF False]]]
    by auto
  have number_of_comps_diff: "card (comps \<V> (to_graph (\<FF>_imp initial))) -
    card (comps \<V> (to_graph (\<FF>_imp (prod.snd ((orlins_one_step_time_check ^^ I) (loopBtime initial)))))) \<le> N - 1" 
  ...
  proof(rule, goal_cases)
    case 1
    have "prod.fst final + prod.fst (loopBtime initial) = 
          t\<^sub>O\<^sub>C  + prod.fst ((orlins_one_step_time_check ^^ I) (loopBtime initial))"
      using time_swap 
      by (simp add:  add_fst_def assms sym[OF to_time])
    also have "... \<le>  t\<^sub>O\<^sub>C + ((card (comps \<V> (to_graph (\<FF>_imp initial))) -
    card (comps \<V> (to_graph (\<FF>_imp (prod.snd ((orlins_one_step_time_check ^^ I) (loopBtime initial))))))) *
   (t\<^sub>A\<^sub>u\<^sub>f + t\<^sub>A\<^sub>C + t\<^sub>A\<^sub>B + t\<^sub>B\<^sub>C + t\<^sub>B\<^sub>B + t\<^sub>B\<^sub>u\<^sub>f) +
   I * (t\<^sub>B\<^sub>F + t\<^sub>B\<^sub>C + t\<^sub>B\<^sub>u\<^sub>f + t\<^sub>A\<^sub>u\<^sub>f + t\<^sub>A\<^sub>C + t\<^sub>O\<^sub>C + t\<^sub>O\<^sub>B) +
   ((\<Sum>j = 1..I.
        let state' = (orlins_one_step_check ^^ (j - 1)) (local.loopB initial) in card {v. important state' v}) +

...
lemma send_flow_succ_condE:" send_flow_succ_cond state \<Longrightarrow> (
                    ...
                    test_all_vertices_zero_balance state \<Longrightarrow> P) \<Longrightarrow> P"
  unfolding  send_flow_succ_cond_def by presburger

lemma send_flow_succ_condI:"  f = current_flow state \<Longrightarrow>
                    b = balance state \<Longrightarrow>
                    \<gamma> = current_\<gamma> state \<Longrightarrow>
                    test_all_vertices_zero_balance state \<Longrightarrow> send_flow_succ_cond state"
  unfolding  send_flow_succ_cond_def by presburger
...
lemma send_flow_call1_condE: 
  assumes "send_flow_call1_cond state"
          "( \<And> f b \<gamma> s t P f' b' state'. f = current_flow state \<Longrightarrow>
                  ...
                  get_source_target_path_a state s = Some (t, P) \<Longrightarrow>
                       f' = augment_edges_impl f \<gamma> P \<Longrightarrow>
                        b' = move b \<gamma> s t \<Longrightarrow>
              state' = state \<lparr> current_flow := f', balance := b'\<rparr> \<Longrightarrow> PP)"
        shows PP 
  ...
  using assms(1)
  by(auto simp add: send_flow_call1_cond_def if_split intro!: assms(2))
...
lemma send_flow_call2_condE: 
  assumes "send_flow_call2_cond state" 
          "( \<And> f b \<gamma> t s P f' b' state'.
                    ...
                    \<gamma> = current_\<gamma> state \<Longrightarrow>
                    \<not> test_all_vertices_zero_balance state \<Longrightarrow>
                    get_source state = None \<Longrightarrow>
                    get_target state = Some t \<Longrightarrow>
                    get_source_target_path_b state t = Some (s, P) \<Longrightarrow>
                       f' = augment_edges_impl f \<gamma> P \<Longrightarrow>
                        b' = move b \<gamma> s t \<Longrightarrow>
                       state' = state \<lparr> current_flow := f', balance := b'\<rparr>
                     \<Longrightarrow> PP)"
...
lemma send_flow_fail1_condE: 
  assumes "send_flow_fail1_cond state"
          "( \<And> f b \<gamma> s. f = current_flow state \<Longrightarrow>
                   ...
                   \<not> test_all_vertices_zero_balance state \<Longrightarrow>
                  get_source state = Some s\<Longrightarrow>
                  get_source_target_path_a state s = None  \<Longrightarrow> PP)"
        shows PP 
  apply(all \<open>cases "test_all_vertices_zero_balance state"\<close>)
  apply( all \<open>cases "get_source state"\<close>)
  apply(all \<open>cases "get_source_target_path_a state (the (get_source state))"\<close>)
...
proof(induction rule: send_flow_induct[OF assms])
  ...
  proof(cases state rule: send_flow_cases)
    ...
    show ?thesis 
      ...
      apply(rule trans[OF _ IH(2)])
      apply(subst send_flow_impl.simps)
      by(auto intro: send_flow_call1_condE[OF 3] 
  ...
  next
    ...
    show ?thesis
      ...
      apply(rule trans[OF _ IH(3)])
      apply(subst send_flow_impl.simps)
      by(auto intro: send_flow_call2_condE[OF 4] 
...
lemma get_target_condI: "\<lbrakk>b = balance state ; \<gamma> = current_\<gamma> state; Some t = get_target state;
...
implementation_invar state\<rbrakk> \<Longrightarrow> P) \<Longrightarrow> P"
  by(auto simp add: get_target_cond_def)
...
"b = balance state \<Longrightarrow> \<gamma> = current_\<gamma> state\<Longrightarrow>
      implementation_invar state \<Longrightarrow> vertex_selection_cond state b \<gamma>"
  by(auto simp add: vertex_selection_cond_def)

lemma vertex_selection_condE: 
"vertex_selection_cond state b \<gamma> \<Longrightarrow> 
(b = balance state \<Longrightarrow> \<gamma> = current_\<gamma> state\<Longrightarrow>
      implementation_invar state \<Longrightarrow> P) \<Longrightarrow> P"
  by(auto simp add: vertex_selection_cond_def)

end

locale send_flow_reasoning = 
send_flow_spec +
algo +
      ...
      show ?thesis
        ...
        subgoal 
          ...
          by (auto intro!: exI[of _ "x+1"] 
                    simp add: send_flow_call1_upd_changes(4) f'_is sym[OF f'] 1   distrib_left mult.commute)  
         ...
         by (auto  intro: exI[of _ "x-1"] disjE[where P="F e \<notin> set P \<and> B e \<in> set P"]
            simp add: left_diff_distrib' 1 sym[OF f'] send_flow_call1_upd_changes(4))      
    next
      case False
      thus ?thesis
    ...
    proof(cases "e \<in> oedge ` set P")
      ...
      show ?thesis
        ...
        subgoal 
          ...
          by (auto intro!: exI[of _ "x+1"] simp add: distrib_left mult.commute  1 sym[OF f'] send_flow_call2_upd_changes(4))
          ...
          by(auto intro:  disjE[where P="F e \<notin> set P \<and> B e \<in> set P"] exI[of _ "x-1"] simp add: left_diff_distrib'  
                    1 sym[OF f'] send_flow_call2_upd_changes(4))
    next
      case False
      thus ?thesis
        using e_not_in_es_flow_not_change[of P e  "abstract_flow_map f" \<gamma>] x_prop f'_is
...
lemma
  ...
  shows   invar_above_6Ngamma_pos_flow_F:
          "pos_flow_F state"
...
proof-
  have gamma_0: "current_\<gamma> state > 0" 
    ...
    have gamma_Phi_flow: "e \<in> \<F> (send_flow_call1_upd state) \<Longrightarrow>
         ...
         apply(rule order.strict_trans2) defer
         apply(rule send_flow_call1_cond_flow_Phi[OF assms(1,3,2,4) _ assms(5) refl])
       ...
       by (auto  simp add: left_diff_distrib' intro: invar_above_6Ngamma_pos_flow_F) 
...
proof-
  have gamma_0: "current_\<gamma> state > 0" 
    ...
    have gamma_Phi_flow: "e \<in> \<F> (send_flow_call2_upd state) \<Longrightarrow>
         ...
         apply(rule order.strict_trans2) defer
         apply(rule send_flow_call2_cond_flow_Phi[OF assms(1,3,2,4) _ assms(5) refl])
...
lemma all_bal_zero_send_flow_dom:
       "implementation_invar state \<Longrightarrow>
        (\<forall>v\<in>\<V>. a_balance state v = 0) \<Longrightarrow> send_flow_dom state"
         "implementation_invar state \<Longrightarrow>
        (\<forall>v\<in>\<V>. a_balance state v = 0) \<Longrightarrow> send_flow_succ_cond state"
...
proof(induction rule: send_flow_induct[OF dom])
  ...
  proof(cases state rule: send_flow_cases)
    case 1
    show ?thesis
      apply(rule send_flow_cont_condE[OF 1])
      using if_send_flow_cont_cond_basic(1,2,3)[OF 1 IH(6)]
      by (force intro!:  orlins_entryI ordered_ab_group_add_abs_class.abs_leI invar_non_zero_bI
             simp add: send_flow_changes_current_\<gamma>[OF IH(1)]  send_flow_simps'(2)[OF 1]
                  send_flow_cont_upd_changes(4,9) algebra_simps)+
  next
    case 2
    thus ?thesis
      using IH(10)
      by(auto simp add:  send_flow_simps'(1)[OF 2] send_flow_succ_upd_def)
  next
    case 3
    have pos_flow_F: "pos_flow_F state" 
      using "3" IH(2-) 
  ...
  next
    ...
    thus ?thesis
      using IH(10)
      by(auto simp add:  send_flow_simps'(3)[OF 5] send_flow_fail_upd_def)
  next
    case 6
    thus ?thesis
      using IH(10)
      by(auto simp add:  send_flow_simps'(4)[OF 6] send_flow_fail_upd_def)
  qed
qed
...
proof(induction rule: send_flow_induct[OF dom])
  ...
  proof(cases state rule: send_flow_cases)
    ...
    show ?thesis
      using IH(10)
      by(auto simp add:  send_flow_simps'(2)[OF 1] send_flow_cont_upd_def)
  next
    case 2
    thus ?thesis
      using IH(10) if_send_flow_succ_cond[OF 2 IH(6)]
      by(auto simp add:  send_flow_simps'(1)[OF 2] send_flow_succ_upd_changes)
  next
    case 3
    have pos_flow_F: "pos_flow_F state" 
      using "3" IH(2-) 
  ...
  next
    case 5
     have pos_flow_F: "pos_flow_F state" 
      ...
      by(auto intro!: invar_above_6Ngamma_pos_flow_F) 
    show ?thesis
      apply(rule disjI1, rule send_flow_fail1_condE[OF 5])
      ...
      by (force intro:  bexI[of _ "the (get_source state)"] simp add: IH(10) send_flow_simps'(3)[OF 5]  send_flow_fail_upd_changes)+
   next
    case 6
     have pos_flow_F: "pos_flow_F state" 
       using IH(2-) 
      by(auto intro!: invar_above_6Ngamma_pos_flow_F) 
    show ?thesis
      apply(rule disjI2, rule send_flow_fail2_condE[OF 6])
      ...
      by (force intro:  bexI[of _ "the (get_target state)"] 
            simp add: IH(10) send_flow_simps'(4)[OF 6]  send_flow_fail_upd_changes )+
  qed
 qed
qed
...
proof-
  ...
  proof(rule disjE[OF s_and_t], goal_cases)
    ...
    also have "... > (N - 1) * ( - \<epsilon> * current_\<gamma> state') + (1 - \<epsilon>) * current_\<gamma> state' "
      ...
      subgoal
        ...
        by(auto simp add: Rescut_def)
      using s_prop  Rescut_around_in_V[of s "a_current_flow state'"] gamma_0
            \<epsilon>_axiom \<V>_finite card_Diff_singleton
    unfolding N_def 
  ...
  next
    ...
    also have "... < (N - 1) * ( \<epsilon> * current_\<gamma> state') + (- (1 - \<epsilon>) * current_\<gamma> state') "
      ...
      using  ARescut_def  N_def  t_prop  ARescut_around_in_V[of t "a_current_flow state'"]
...
proof(subst send_flow_cases[of state], goal_cases)
  ...
  thus ?case
    by (auto intro: send_flow_cont_condE 
          simp add: send_flow_cont_upd_def send_flow_simps'(2) )
next
  case 2
...
next
  case 3
  thus ?case
    using  if_send_flow_call1_cond_basic[OF 3 assms(3)] assms(2,4)
    by(auto elim!: send_flow_call1_condE invar_gammaE simp add: algebra_simps)(smt (verit))
next
  case 4
  thus ?case
    using  if_send_flow_call2_cond_basic[OF 4 assms(3)] assms(2,4)
    by(auto elim!: send_flow_call2_condE invar_gammaE simp add: algebra_simps)(smt (verit))
next
  case 5
  thus ?case
    using  if_send_flow_fail1_cond_basic[OF 5 assms(3)] assms(2,4)
    by(auto elim!: send_flow_fail1_condE invar_gammaE simp add: algebra_simps)(smt (verit))
next
  case 6
  thus ?case
    using  if_send_flow_fail2_cond_basic[OF 6 assms(3)] assms(2,4)
    by(auto elim!: send_flow_fail2_condE invar_gammaE simp add: algebra_simps)(smt (verit))
qed simp

...
proof -
  ...
  then have "\<forall>y. y \<in> ?set_diff \<longrightarrow> List.member l y" using  \<open>length l > 0\<close> by simp
  ...
  proof (rule ccontr)
    ...
    then have "List.member (take y l) z" using  z_prop2 by simp
          ...
          proof
            ...
            proof
              ...
              then have assum3: "List.member (take (length l - 1) l) (nth l (length l - 1))" 
                using  \<open>l \<noteq> []\<close> assum2 by simp
      ...
      proof -
        ...
        next
          ...
          next
            case False
            have "l \<noteq> []" using \<open>X \<noteq> {}\<close> \<open>set l = X\<close> by auto
            have "List.member l (nth l 0)" using \<open>set l = X\<close> \<open>set l = X\<close> \<open>l \<noteq> []\<close> by fastforce
            then have "nth l 0 \<in> X" using \<open>set l = X\<close>  by simp

...
function (domintros) matroid_intersection::"'mset \<Rightarrow> 'mset"  where
"matroid_intersection X =
 (let (SX, TX, graph) = compute_graph X (complement X) in
     (case find_path SX TX graph of 
          None \<Rightarrow> X |
          Some p \<Rightarrow> matroid_intersection (augment X p) ))"
  by pat_completeness auto

definition "matroid_intersection_recurse_cond X =
(let (SX, TX, graph) = compute_graph X (complement X) in
     (case find_path SX TX graph of 
...
"matroid_intersection_recurse_cond X \<Longrightarrow>
 (\<And> SX TX graph p. compute_graph X (complement X) = (SX, TX, graph) ==>
         find_path SX TX graph = Some p \<Longrightarrow> P) ==> P"
  by(force simp add: matroid_intersection_recurse_cond_def Let_def)

definition "matroid_intersection_recurse_upd X =
(let (SX, TX, graph) = compute_graph X (complement X) in
     augment X(the ( find_path SX TX graph )))"

lemma P_of_matroid_intersection_recurseI:
"matroid_intersection_recurse_cond X \<Longrightarrow> (\<And> SX TX graph p. 
         compute_graph X (complement X) = (SX, TX, graph) ==>
         find_path SX TX graph = Some p \<Longrightarrow> P (augment X p)) ==> P (matroid_intersection_recurse_upd X)"
  unfolding matroid_intersection_recurse_cond_def
                       matroid_intersection_recurse_upd_def Let_def 
  apply(cases "compute_graph X (complement X)", simp)
  subgoal for a b c
    by(cases "find_path a b c") auto
  done

definition "matroid_intersection_terminates_cond X =
(let (SX, TX, graph) = compute_graph X (complement X) in
     (case find_path SX TX graph of 
...
"matroid_intersection_terminates_cond X \<Longrightarrow>
 (\<And> SX TX graph.  compute_graph X (complement X) = (SX, TX, graph) ==>
...
"(matroid_intersection_terminates_cond X \<Longrightarrow> P) \<Longrightarrow>
 (matroid_intersection_recurse_cond X \<Longrightarrow> P) \<Longrightarrow> P"
  unfolding matroid_intersection_terminates_cond_def matroid_intersection_recurse_cond_def Let_def
  apply(cases "compute_graph X (complement X)")
  subgoal for a b c
    by(cases "find_path a b c") auto
  done

lemma matroid_intersection_simps:
  assumes "matroid_intersection_dom X"
  shows "matroid_intersection_terminates_cond X \<Longrightarrow> matroid_intersection X = X"
        "matroid_intersection_recurse_cond X \<Longrightarrow>
           matroid_intersection X = matroid_intersection (matroid_intersection_recurse_upd X)"  
...
lemma matroid_intersection_induct:
  assumes "matroid_intersection_dom X"
          "\<And> X. matroid_intersection_dom X  \<Longrightarrow>
                 (matroid_intersection_recurse_cond X \<Longrightarrow> P (matroid_intersection_recurse_upd X)) \<Longrightarrow> P X"
  shows   "P X"
...
partial_function (tailrec) matroid_intersection_impl::"'mset \<Rightarrow> 'mset" where
"matroid_intersection_impl X =
 (let (SX, TX, graph) = compute_graph X (complement X) in
     (case find_path SX TX graph of 
          None \<Rightarrow> X |
          Some p \<Rightarrow> matroid_intersection_impl (augment X p)))"

lemma implementation_is_same:
  "matroid_intersection_dom X \<Longrightarrow> matroid_intersection_impl X = matroid_intersection X"
  apply(induction X rule: matroid_intersection.pinduct)
...
function (domintros) matroid_intersection_circuit::"'mset \<Rightarrow> 'mset"  where
"matroid_intersection_circuit X =
 (let (SX, TX, graph) = compute_graph_circuit X (complement X) in
     (case find_path SX TX graph of 
          None \<Rightarrow> X |
          Some p \<Rightarrow> matroid_intersection_circuit (augment X p) ))"
  by pat_completeness auto

definition "matroid_intersection_circuit_recurse_cond X =
(let (SX, TX, graph) = compute_graph_circuit X (complement X) in
     (case find_path SX TX graph of 
          None \<Rightarrow> False |
          Some p \<Rightarrow> True))"
...
"matroid_intersection_circuit_recurse_cond X \<Longrightarrow>
 (\<And> SX TX graph p. compute_graph_circuit X (complement X) = (SX, TX, graph) ==>
         find_path SX TX graph = Some p \<Longrightarrow> P) ==> P"
  by(force simp add: matroid_intersection_circuit_recurse_cond_def Let_def)

definition "matroid_intersection_circuit_recurse_upd X =
(let (SX, TX, graph) = compute_graph_circuit X (complement X) in
     augment X(the ( find_path SX TX graph )))"

lemma P_of_matroid_intersection_circuit_recurseI:
"matroid_intersection_circuit_recurse_cond X \<Longrightarrow> (\<And> SX TX graph p. 
         compute_graph_circuit X (complement X) = (SX, TX, graph) ==>
         find_path SX TX graph = Some p \<Longrightarrow> P (augment X p)) ==> P (matroid_intersection_circuit_recurse_upd X)"
  unfolding matroid_intersection_circuit_recurse_cond_def
                       matroid_intersection_circuit_recurse_upd_def Let_def 
  apply(cases "compute_graph_circuit X (complement X)", simp)
  subgoal for a b c
    by(cases "find_path a b c") auto
  done

definition "matroid_intersection_circuit_terminates_cond X =
(let (SX, TX, graph) = compute_graph_circuit X (complement X) in
     (case find_path SX TX graph of 
          None \<Rightarrow> True |
          Some p \<Rightarrow> False))"
...
"matroid_intersection_circuit_terminates_cond X \<Longrightarrow>
 (\<And> SX TX graph.  compute_graph_circuit X (complement X) = (SX, TX, graph) ==>
         find_path SX TX graph = None \<Longrightarrow> P) ==> P"
  by(force simp add: matroid_intersection_circuit_terminates_cond_def Let_def)

lemma matroid_intersection_circuit_cases:
"(matroid_intersection_circuit_terminates_cond X \<Longrightarrow> P) \<Longrightarrow>
 (matroid_intersection_circuit_recurse_cond X \<Longrightarrow> P) \<Longrightarrow> P"
  unfolding matroid_intersection_circuit_terminates_cond_def 
       matroid_intersection_circuit_recurse_cond_def Let_def
  apply(cases "compute_graph_circuit X (complement X)")
  subgoal for a b c
    by(cases "find_path a b c") auto
  done

lemma matroid_intersection_circuit_simps:
  assumes "matroid_intersection_circuit_dom X"
  shows "matroid_intersection_circuit_terminates_cond X \<Longrightarrow> matroid_intersection_circuit X = X"
        "matroid_intersection_circuit_recurse_cond X \<Longrightarrow>
           matroid_intersection_circuit X = matroid_intersection_circuit
           (matroid_intersection_circuit_recurse_upd X)"  
...
lemma matroid_intersection_circuit_induct:
  assumes "matroid_intersection_circuit_dom X"
          "\<And> X. matroid_intersection_circuit_dom X  \<Longrightarrow>
                 (matroid_intersection_circuit_recurse_cond X
               \<Longrightarrow> P (matroid_intersection_circuit_recurse_upd X)) \<Longrightarrow> P X"
  shows   "P X"
...
partial_function (tailrec) matroid_intersection_circuit_impl::"'mset \<Rightarrow> 'mset" where
"matroid_intersection_circuit_impl X =
 (let (SX, TX, graph) = compute_graph_circuit X (complement X) in
     (case find_path SX TX graph of 
          None \<Rightarrow> X |
          Some p \<Rightarrow> matroid_intersection_circuit_impl (augment X p)))"

lemma implementation_circuit_is_same:
  "matroid_intersection_circuit_dom X 
    \<Longrightarrow> matroid_intersection_circuit_impl X = matroid_intersection_circuit X"
  apply(induction X rule: matroid_intersection_circuit.pinduct)
...
assumes inner_fold: "\<And> X f G. set_invar X \<Longrightarrow> \<exists> xs. set xs = to_set X
                                           \<and> distinct xs \<and>
                                            inner_fold X f G = foldr f xs G"
assumes inner_fold_circuit: "\<And> X f G. set_invar_red X \<Longrightarrow> \<exists> xs. set xs = to_set_red X
                                           \<and> distinct xs \<and>
                                            inner_fold_circuit X f G = foldr f xs G"
assumes outer_fold: "\<And> X f trip. set_invar_red X \<Longrightarrow> \<exists> xs. set xs = to_set_red X
                                           \<and> distinct xs \<and>
                                            outer_fold X f trip = foldr f xs trip"
assumes find_path: "\<And> G S T. graph.graph_inv G \<Longrightarrow> graph.finite_graph G \<Longrightarrow> graph.finite_vsets \<Longrightarrow>
                             ...
                             (\<nexists> p u v. vwalk_bet_single_vertex (graph.digraph_abs G) u p v \<and>
                                       u \<in> vset S \<and> v \<in> vset T)"
                   "\<And> G S T p. graph.graph_inv G \<Longrightarrow> graph.finite_graph G \<Longrightarrow> graph.finite_vsets \<Longrightarrow>
                             vset_inv S \<Longrightarrow> vset_inv T \<Longrightarrow> vset S \<subseteq> carrier \<Longrightarrow>
                             vset T \<subseteq> carrier \<Longrightarrow> dVs (graph.digraph_abs G) \<subseteq> carrier \<Longrightarrow>
                             find_path S T G = Some p \<Longrightarrow>
                             \<exists> u v. vwalk_bet_single_vertex (graph.digraph_abs G) u p v \<and> u \<in> vset S \<and> v \<in> vset T \<and>
                             (\<nexists> p'. vwalk_bet_single_vertex (graph.digraph_abs G) u p' v \<and> length p' < length p)"
  ...
  assumes finite_vsets: "graph.finite_vsets"
...
assumes circuit2: "\<And> X y. set_invar X \<Longrightarrow> indep1 (to_set X) \<Longrightarrow> to_set X \<subseteq> carrier \<Longrightarrow>
                             y \<in> carrier \<Longrightarrow>
...
proof-
  ...
  obtain xs where xs_prop: "set xs = to_set X" "distinct xs" "inner_fold X f G = foldr f xs G"
...
proof-
  define f where "f = (\<lambda> y (SX, TX, current_map). (
            ...
            in (SX, TX, current_map)))"
  obtain xs where xs_prop: "set xs = to_set_red E_without_X" "distinct xs"
...
definition "indep_invar X = (indep1 (to_set X) \<and> (indep2 (to_set X)))"

lemma indep_invar_recurse_improvement:
  assumes "matroid_intersection_recurse_cond X" "indep_invar X"
          "set_invar X" "to_set X \<subseteq> carrier"
  shows   "indep_invar (matroid_intersection_recurse_upd X)"
          "set_invar (matroid_intersection_recurse_upd X)" 
          "to_set (matroid_intersection_recurse_upd X) \<subseteq> carrier"
          "card (to_set (matroid_intersection_recurse_upd X)) = 
          card (to_set X) + 1"
          "S (to_set X) \<noteq> {}"
          "T (to_set X) \<noteq> {}"
    and   "\<exists>u v p. vwalk_bet_single_vertex (A1 (to_set X) \<union> A2 (to_set X)) u p v \<and>
           u \<in> S (to_set X) \<and>
           v \<in> T (to_set X)" (is ?last_thesis)
proof(all \<open>rule P_of_matroid_intersection_recurseI[OF assms(1)]\<close>)
  fix SX TX graph p
  assume 1: "compute_graph X (complement X) = (SX, TX, graph)"
            "find_path SX TX graph = Some p"
  have Xincarrier: "to_set X \<subseteq> carrier"
    by (simp add: assms(4))
  have indep1X: "indep1 (to_set X)"
    using assms(2) indep_invar_def by auto
  have indep2X: "indep2 (to_set X)" 
    using assms(2) indep_invar_def by auto
  have complementInvar: "set_invar_red (complement X)" 
    using complement(1)[OF assms(3) Xincarrier] by simp
  hence complement_is: "to_set_red (complement X) = carrier - to_set X"
   by (simp add: Xincarrier assms(3) complement(2))
  hence complementcarrier: "to_set_red (complement X) \<subseteq> carrier - to_set X"
    by simp
  note compute_graph_correct = compute_graph_correct[OF indep1X indep2X assms(3) 
                                 complementInvar complementcarrier 1(1)]
  note compute_graph_meaning = compute_graph_meaning[OF indep1X indep2X assms(3) complementInvar complement_is 1(1)]
  ...
  obtain u v where p_prop: "vwalk_bet_single_vertex [graph]\<^sub>g u p v"
      "u \<in> vset SX"
      "v \<in> vset TX"
      "(\<nexists>p'.
          vwalk_bet_single_vertex [graph]\<^sub>g u p' v \<and> length p' < length p)"
  ...
  hence p_prop_extended: "vwalk_bet_single_vertex (A1 (to_set X) \<union> A2 (to_set X)) u p v" 
             "(\<nexists>p'.
          vwalk_bet_single_vertex (A1 (to_set X) \<union> A2 (to_set X)) u p' v \<and> length p' < length p)"
    using compute_graph_meaning(3) p_prop(4) by auto
  ...
  have p_not_empty:"p \<noteq> []" 
    using p_prop(1) by(auto simp add: vwalk_bet_single_vertex_def vwalk_bet_def) 
  ...
  proof(rule ccontr, goal_cases)
    ...
    moreover hence "vwalk_bet [graph]\<^sub>g u p v"
      using p_prop(1) by(auto simp add: vwalk_bet_single_vertex_def)
    ultimately have "vwalk_bet [graph]\<^sub>g u (p1@[x]@p3) v"
      by (meson indep1X indep2X vwalk_bet_cycle_delete)
    hence  "vwalk_bet_single_vertex [graph]\<^sub>g u (p1@[x]@p3) v"
  ...
  have indep_after_aug:"indep1 (to_set (augment X p))"
    ...
    by(subst effect_of_augmentation(2)[OF  assms(3)  Xincarrier pincarrier  distinctp refl])
      (auto intro: vwalk_arcs.cases[of p] simp add: p_not_empty vwalk_bet_single_vertex_def vwalk_bet_def)  
  moreover have "indep2 (to_set (augment X p))"
    ...
    by(subst effect_of_augmentation(2)[OF  assms(3)  Xincarrier pincarrier  distinctp refl])
      (auto intro: vwalk_arcs.cases[of p] simp add: p_not_empty vwalk_bet_single_vertex_def vwalk_bet_def)  
  ultimately show "indep_invar (augment X p)"
    by(simp add: indep_invar_def)
  show "set_invar (augment X p)"
    by (simp add: Xincarrier assms(3) distinctp effect_of_augmentation(1) pincarrier)
  show "to_set (augment X p) \<subseteq> carrier"
    by (simp add: indep_after_aug matroid1.indep_subset_carrier)
  show "card (to_set (augment X p)) = card (to_set X) + 1"
    ...
    by(subst effect_of_augmentation(2)[OF  assms(3)  Xincarrier pincarrier  distinctp refl])
      (auto intro: vwalk_arcs.cases[of p] simp add: p_not_empty vwalk_bet_single_vertex_def vwalk_bet_def)
  show ?last_thesis 
    using compute_graph_meaning p_prop p_prop_extended(1) by auto
  thus "S (to_set X) \<noteq> {}" "T (to_set X) \<noteq> {}" by auto
qed

lemma indep_invar_max_found:
  assumes "matroid_intersection_terminates_cond X" "indep_invar X"
          "set_invar X" "to_set X \<subseteq> carrier"
  shows   "\<nexists> Y. indep1 Y \<and> indep2 Y \<and> card Y > card (to_set X)"
proof(rule matroid_intersection_terminates_condE[OF assms(1)])
  fix SX TX graph p
  assume 1: "compute_graph X (complement X) = (SX, TX, graph)"
            "find_path SX TX graph = None"
  have Xincarrier: "to_set X \<subseteq> carrier"
    by (simp add: assms(4))
  have indep1X: "indep1 (to_set X)"
    using assms(2) indep_invar_def by auto
  ...
  have graphverticesincarrier: "dVs [graph]\<^sub>g \<subseteq> carrier" 
    using compute_graph_meaning(3) dVs_A1A2_carrier indep1X indep2X by presburger
  have no_path: "(\<nexists>p u v. vwalk_bet_single_vertex [graph]\<^sub>g u p v \<and> u \<in> vset SX \<and> v \<in> vset TX)"
  ...
  hence "\<nexists>p x y. x \<in> S (to_set X) \<and> y \<in> T (to_set X) \<and> (vwalk_bet (A1 (to_set X) \<union> A2 (to_set X)) x p y \<or> x = y)"
    ...
    by (auto simp add: vwalk_bet_single_vertex_def vwalk_bet_def)
  thus  "\<nexists>Y. indep1 Y \<and> indep2 Y \<and> card (to_set X) < card Y"
    by(rule if_no_augpath_then_maximum(1)[OF indep1X indep2X _ refl])
qed

lemma matroid_intersection_terminates_general:
  assumes  "indep_invar X" "set_invar X" "to_set X \<subseteq> carrier"
           "m = card carrier - card (to_set X)"
  shows    "matroid_intersection_dom X"
  using assms
proof(induction m arbitrary: X)
  case 0
  hence X_is_carrier:"to_set X = carrier" 
    by (simp add: card_seteq matroid2.carrier_finite)
  show ?case 
  proof(cases X rule: matroid_intersection_cases)
    case 1
    show ?thesis 
  ...
  next
    case 2
    have "S (to_set X) \<noteq> {}" "T (to_set X) \<noteq> {}"
      using indep_invar_recurse_improvement(7)[OF 2 0(1,2,3)] by auto
    moreover have  "S (to_set X) = {}"
...
next
  ...
  next
    ...
    proof(rule matroid_intersection_recurse_condE[OF 2], goal_cases)
      case (1 SX TX graph p)
      have helper:"indep_invar (augment X p)" "set_invar (augment X p)"
                  "to_set (augment X p) \<subseteq> carrier"
...
lemma matroid_intersection_correctness_general:
  assumes "indep_invar X" "set_invar X" "to_set X \<subseteq> carrier"
    shows "indep_invar (matroid_intersection X) \<and> 
            (\<nexists>Y. indep1 Y \<and> indep2 Y \<and> card (to_set (matroid_intersection X)) < card Y)"
  using assms
proof(induction X rule: matroid_intersection_induct)
  case 1
  then show ?case 
...
lemma matroid_intersection_correctness:
        "indep_invar (matroid_intersection set_empty)"
        "(\<nexists>Y. indep1 Y \<and> indep2 Y \<and> card (to_set (matroid_intersection set_empty)) < card Y)"
  by(rule conjunct1[OF matroid_intersection_correctness_general]|
     rule conjunct2[OF matroid_intersection_correctness_general]|
     fastforce simp add: indep_invar_def set_empty)+

lemma matroid_intersection_termiantes: "matroid_intersection_dom set_empty"
  by(rule matroid_intersection_terminates_general)
    (auto simp add: indep_invar_def set_empty)
...
proof-
  ...
  obtain xs where xs_prop: "set xs = to_set_red X" "distinct xs" "inner_fold_circuit X f G = foldr f xs G"
...
lemma indep_invar_recurse_circuit_improvement:
  assumes "matroid_intersection_circuit_recurse_cond X" "indep_invar X"
          "set_invar X" "to_set X \<subseteq> carrier"
  shows   "indep_invar (matroid_intersection_circuit_recurse_upd X)"
          "set_invar (matroid_intersection_circuit_recurse_upd X)" 
          "to_set (matroid_intersection_circuit_recurse_upd X) \<subseteq> carrier"
          "card (to_set (matroid_intersection_circuit_recurse_upd X)) = 
          card (to_set X) + 1"
    and   "\<exists>u v p. vwalk_bet_single_vertex (A1 (to_set X) \<union> A2 (to_set X)) u p v \<and>
           u \<in> S (to_set X) \<and>
           v \<in> T (to_set X)" (is ?last_thesis)
proof(all \<open>rule P_of_matroid_intersection_circuit_recurseI[OF assms(1)]\<close>)
  ...
  hence complementcarrier: "to_set_red (complement X) \<subseteq> carrier - to_set X"
    by simp
  note compute_graph_correct = compute_graph_circuit_correct[OF indep1X indep2X assms(3) 
                                 complementInvar complementcarrier 1(1)]
  have SXincarrier: "vset SX \<subseteq> carrier" 
    using complementcarrier compute_graph_correct(5) by blast
  have TXincarrier: "vset TX \<subseteq> carrier"
    using complementcarrier compute_graph_correct(6) by blast
  have graphverticesincarrier: "dVs [graph]\<^sub>g \<subseteq> carrier" 
  ...
  hence p_prop_extended: "vwalk_bet_single_vertex (A1 (to_set X) \<union> A2 (to_set X)) u p v" 
             "(\<nexists>p'.
          vwalk_bet_single_vertex (A1 (to_set X) \<union> A2 (to_set X)) u p' v \<and> length p' < length p)"
    using compute_graph_correct p_prop(4) 
    by (auto simp add: A1_def A2_def complement_is)
  have pincarrier:"set p \<subseteq> carrier"
    using p_prop(1-3)
    using graphverticesincarrier in_mono vwalk_bet_in_vertices[of "[graph]\<^sub>g" u p v] SXincarrier 
    by (auto simp add: vwalk_bet_single_vertex_def)
  ...
  have indep_after_aug:"indep1 (to_set (augment X p))"
    using p_prop_extended compute_graph_meaning_circuit p_prop
      ...
      by (subst effect_of_augmentation(2)[OF  assms(3)  Xincarrier pincarrier  distinctp refl])
         (auto intro: vwalk_arcs.cases[of p] simp add: vwalk_bet_single_vertex_def S_def complement_is compute_graph_correct(5)
  ...
  show "to_set (augment X p) \<subseteq> carrier"
    by (simp add: indep_after_aug matroid1.indep_subset_carrier)
  show "card (to_set (augment X p)) = card (to_set X) + 1"
      using p_not_empty  p_prop_extended(1,2)  augment_in_both_matroids_single(3)[OF indep1X indep2X, of u] 
            "1"(1) assms(3) complementInvar complement_is compute_graph_meaning_circuit(1)
            compute_graph_meaning_circuit(2) indep1X indep2X p_prop(2) p_prop(3)   
             augment_in_both_matroids(3)[OF indep1X indep2X _ _ _ _ refl, of u p v]
            compute_graph_correct(5,6) p_prop 
      by(subst effect_of_augmentation(2)[OF  assms(3)  Xincarrier pincarrier  distinctp refl])
        (auto intro: vwalk_arcs.cases[of p] simp add: T_def vwalk_bet_single_vertex_def S_def )
  show ?last_thesis 
    using compute_graph_meaning_circuit p_prop p_prop_extended(1)
           "1"(1) assms(3) complementInvar complement_is indep1X indep2X by blast
qed

lemma indep_invar_max_found_circuit:
  assumes "matroid_intersection_circuit_terminates_cond X" "indep_invar X"
          "set_invar X" "to_set X \<subseteq> carrier"
  shows   "\<nexists> Y. indep1 Y \<and> indep2 Y \<and> card Y > card (to_set X)"
proof(rule matroid_intersection_circuit_terminates_condE[OF assms(1)])
  ...
  note compute_graph_correct = compute_graph_circuit_correct[OF indep1X indep2X assms(3) 
                                 complementInvar complementcarrier 1(1)]
  note compute_graph_meaning = compute_graph_meaning_circuit[OF indep1X indep2X assms(3) complementInvar complement_is 1(1)]
  have SXincarrier: "vset SX \<subseteq> carrier" 
    using S_in_carrier compute_graph_meaning(1) indep1X indep2X by auto
  have TXincarrier: "vset TX \<subseteq> carrier" 
    using T_in_carrier compute_graph_meaning(2) indep1X indep2X by auto
...
lemma matroid_intersection_circuit_terminates_general:
  assumes  "indep_invar X" "set_invar X" "to_set X \<subseteq> carrier"
           "m = card carrier - card (to_set X)"
  shows    "matroid_intersection_circuit_dom X"
  using assms
proof(induction m arbitrary: X)
  case 0
  hence X_is_carrier:"to_set X = carrier" 
    by (simp add: card_seteq matroid2.carrier_finite)
  show ?case 
  proof(cases X rule: matroid_intersection_circuit_cases)
    case 1
    show ?thesis 
...
lemma matroid_intersection_circuit_correctness_general:
  assumes "indep_invar X" "set_invar X" "to_set X \<subseteq> carrier"
    shows "indep_invar (matroid_intersection_circuit X) \<and> 
            (\<nexists>Y. indep1 Y \<and> indep2 Y \<and> card (to_set (matroid_intersection_circuit X)) < card Y)"
  using assms
proof(induction X rule: matroid_intersection_circuit_induct)
  case 1
  then show ?case 
...
lemma matroid_intersection_circuit_correctness:
        "indep_invar (matroid_intersection_circuit set_empty)"
        "(\<nexists>Y. indep1 Y \<and> indep2 Y \<and> card (to_set (matroid_intersection_circuit set_empty)) < card Y)"
  by(rule conjunct1[OF matroid_intersection_circuit_correctness_general]|
     rule conjunct2[OF matroid_intersection_circuit_correctness_general]|
     fastforce simp add: indep_invar_def set_empty)+

lemma matroid_intersection_circuit_termiantes: "matroid_intersection_circuit_dom set_empty"

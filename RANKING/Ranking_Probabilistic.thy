...
next
  ...
  next
    case 4
    note \<sigma>'eq = \<open>\<sigma>' = filter (\<lambda>v. v \<in> X) \<sigma>\<close>
    have "\<sigma>' = filter (\<lambda>v. v \<in> X) (x # tl \<sigma>)"
      unfolding \<sigma>_tl[symmetric] by (fact \<sigma>'eq)
    also have "\<dots> = filter (\<lambda>v. v \<in> X) (tl \<sigma>)"
      using \<open>x \<notin> X\<close> by simp
    finally show ?case .
  next
    case 5

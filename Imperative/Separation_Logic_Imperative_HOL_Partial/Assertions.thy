...
lemma mod_h_bot_normalize[simp]: 
  "NO_MATCH undefined h \<Longrightarrow> (h,{})\<Turnstile>P \<longleftrightarrow> h\<^sub>\<bottom> \<Turnstile> P"
...
named_theorems models_simps \<open>Simp-rules for the models predicate\<close>

lemma mod_true[models_simps]: "h\<Turnstile>true \<longleftrightarrow> in_range h"
...
lemma mod_pure[models_simps]: "h\<Turnstile>\<up>b \<longleftrightarrow> snd h = {} \<and> b"
  unfolding pure_assn_def 
  apply (cases h) 
  apply (auto simp add: Abs_assn_inverse)
  done

lemma mod_ex_dist[models_simps]: "h\<Turnstile>(\<exists>\<^sub>Ax. P x) \<longleftrightarrow> (\<exists>x. h\<Turnstile>P x)"
  unfolding ex_assn_def by (auto simp: Abs_assn_inverse)
  
lemma mod_exI: "h\<Turnstile>P x \<Longrightarrow> h\<Turnstile>(\<exists>\<^sub>Ax. P x)"
  by (auto simp: models_simps)
...
lemma mod_or_dist: "h\<Turnstile>P\<or>\<^sub>AQ \<longleftrightarrow> h\<Turnstile>P \<or> h\<Turnstile>Q"
  unfolding sup_assn_def by (simp add: Abs_assn_inverse)

lemma mod_not_dist: "h\<Turnstile>(\<not>\<^sub>AP) \<longleftrightarrow> in_range h \<and> \<not> h\<Turnstile>P"
  unfolding uminus_assn_def by (simp add: Abs_assn_inverse)

lemma mod_pure_star_dist: "h\<Turnstile>P*\<up>b \<longleftrightarrow> h\<Turnstile>P \<and> b"
  by (metis (full_types) mod_false mult_1_right pure_false 
    pure_true star_false_right)

lemmas mod_dist = mod_pure_star_dist (*mod_ex_dist*) mod_and_dist
...
lemma ent_true[simp]: "P \<Longrightarrow>\<^sub>A true"
  by (auto intro!: entailsI simp: models_in_range models_simps)
...
lemma ent_pure_post_iff[simp]: 
  "(P \<Longrightarrow>\<^sub>A Q*\<up>b) \<longleftrightarrow> (P \<Longrightarrow>\<^sub>A Q) \<and> (\<forall>h. h\<Turnstile>P \<longrightarrow> b)"
  unfolding entails_def
  by (auto simp add: mod_dist)

lemma ent_pure_post_iff_sng[simp]: 
  "(P \<Longrightarrow>\<^sub>A \<up>b) \<longleftrightarrow> (P \<Longrightarrow>\<^sub>A emp) \<and> (\<forall>h. h\<Turnstile>P \<longrightarrow> b)"
...
lemma ent_disjI1:
  ...
  shows "P \<Longrightarrow>\<^sub>A R" using assms unfolding entails_def 
  by (simp add: models_simps mod_dist)

lemma ent_disjI2:
  assumes "P \<or>\<^sub>A Q \<Longrightarrow>\<^sub>A R" 
  shows "Q \<Longrightarrow>\<^sub>A R" using assms unfolding entails_def 
  by (simp add: models_simps mod_dist)
  

lemma ent_disjI1_direct[simp]: "A \<Longrightarrow>\<^sub>A A \<or>\<^sub>A B"
  by (simp add: models_simps mod_dist entails_def)
...
lemma star_or_dist1: 
  ...
  apply (rule ent_iffI) 
  unfolding entails_def
  by (auto simp add: mod_star_conv models_simps mod_dist) 
...
lemma ent_disjI1': "A\<Longrightarrow>\<^sub>AB \<Longrightarrow> A\<Longrightarrow>\<^sub>AB\<or>\<^sub>AC"
  by (auto simp add: entails_def models_simps mod_dist) 
...
lemma entt_disjI1': "A\<Longrightarrow>\<^sub>tB \<Longrightarrow> A\<Longrightarrow>\<^sub>tB\<or>\<^sub>AC"
  by (auto simp: entailst_def entails_def star_or_dist mod_simps mod_dist)

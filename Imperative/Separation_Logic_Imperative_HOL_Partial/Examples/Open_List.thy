...
interpretation os: imp_list os_list .
...
lemma os_empty_impl: "imp_list_empty os_list os_empty"
  apply unfold_locales
  apply (sep_auto heap: os_empty_rule)
  done
...
lemma os_is_empty_impl: "imp_list_is_empty os_list os_is_empty"
  apply unfold_locales
  apply (sep_auto heap: os_is_empty_rule)
  done
...
lemma os_prepend_impl: "imp_list_prepend os_list os_prepend"
  apply unfold_locales
  apply (sep_auto heap: os_prepend_rule)
  done
...
lemma os_pop_impl: "imp_list_pop os_list os_pop"
  apply unfold_locales
  apply (sep_auto heap: os_pop_rule)
  done
...
next
  ...
  show ?case
    ...
    apply (sep_vcg auto; clarsimp)
    subgoal for pp n h as
      using Cons.hyps[of n "x#ys" p]
      by sep_auto
    done
qed
...
lemma os_reverse_rule: "<os_list xs p> os_reverse p <os_list (rev xs)>"
  ...
  using os_reverse_aux_rule[where ys="[]" and q=None, simplified, rule_format]
  by auto

lemma os_reverse_impl: "imp_list_reverse os_list os_reverse"
  apply unfold_locales
  apply (sep_auto heap: os_reverse_rule)
  done
...
proof (induct xs arbitrary: b x) (* Have to generalize over x, as 
    sep_auto preprocessor introduces a new variable. *)
  case Nil show ?case
    apply sep_auto
    done
next
  case (Cons y xs) 
  show ?case by (sep_auto heap: Cons.hyps)
qed
...
lemma os_iterate_impl: 
  ...
  subgoal by (sep_auto intro': mod_exI[where x="[]"])
  subgoal for l l' p it
    apply (cases it; cases l'; cases l; clarsimp)
    apply (sep_vcg auto)
  ...
  subgoal  
    apply (sep_frame_fwd rule: lseg_conc)
    by sep_auto
  done
interpretation os: 
  imp_list_iterate os_list os_is_it os_it_init os_it_has_next os_it_next
...
lemma os_sum_rule[sep_heap_rules]: 
  "<os_list l p> os_sum p <\<lambda>r. os_list l p * \<up>(r=sum_list l)>"

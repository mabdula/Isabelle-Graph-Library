...
begin
  ...
  *)
  ...
  lemma ias_new_rule: "<emp> ias_new < is_ias {} >"
    supply [sep_heap_rules] = ias_new_sz_rule
    unfolding ias_new_def by (sep_auto)
  ...
  lemma ias_ins_rule:
    "< is_ias m p > ias_ins k p <\<lambda>r. is_ias (insert k m) r>"
  ...
  interpretation ias: imp_set is_ias by unfold_locales
  
  lemma ias_empty_impl: "imp_set_empty is_ias ias_new"
    apply unfold_locales
    by (sep_auto heap: ias_new_rule)
  interpretation ias: imp_set_empty is_ias ias_new by (rule ias_empty_impl) 
  ...
  context begin
    interpretation ias: imp_set_ins is_ias ias_ins proof
      fix s p a
      show "<is_ias s p> ias_ins a p <is_ias (insert a s)>"
        unfolding is_ias_def ias_ins_def
        by (sep_auto simp: ias_ins_abs1 ias_ins_abs2)
    qed    

...
lemma uf_init_rule[sep_heap_rules]: 
  ...
  apply (clarsimp simp: ufa_init_correct ufa_init_invar)
  apply (sep_auto simp: ufa_init_correct ufa_init_invar)
  done
...
next
  ...
  show ?case
    apply (subst uf_compress.simps)
    apply (sep_auto simp: SS  ufa_compress_invar ufa_compress_aux(2) heap: IH')
    done
qed

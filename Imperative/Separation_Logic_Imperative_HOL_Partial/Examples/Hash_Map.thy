...
lemma hm_new_rule': 
  ...
  unfolding is_hashmap'_def
  apply (sep_auto heap: complete_ht_new)
  done 

lemma hm_new_rule: 
  "<emp> hm_new <is_hashmap Map.empty>"
  unfolding is_hashmap'_def is_hashmap_def
  by (sep_auto heap: complete_ht_new)
...
lemma hm_lookup_rule':
  "<is_hashmap' m l ht> hm_lookup k ht 
    <\<lambda>r. is_hashmap' m l ht * 
      \<up>(r = m k)>"
  unfolding is_hashmap'_def
  apply (sep_auto heap: complete_ht_lookup)
  unfolding is_hashtable_def
  apply (sep_auto simp: ls_lookup_map_of)
  done
...
lemma hm_lookup_rule:
  ...
  by (sep_auto heap: hm_lookup_rule')
...
proof (cases "length l * load_factor \<le> the_size ht * 100")
  ...
  show ?thesis
    unfolding is_hashmap'_def
    apply (sep_auto heap: complete_ht_update_rehash[OF True])
    unfolding is_hashmap_def is_hashmap'_def is_hashtable_def
    apply (sep_auto simp: abs_update_rehash_map_of)
    done
next
  case False
  show ?thesis    
    unfolding is_hashmap'_def
    apply (sep_frame_fwd rule: is_hashtable_addI)
    apply (sep_auto heap: complete_ht_update_normal[OF False])
    unfolding is_hashmap_def is_hashmap'_def is_hashtable_def
    apply (sep_auto simp: abs_update_map_of )
    done
qed
...
lemma hm_update_rule:
  ...
  by (sep_auto heap: hm_update_rule')
...
lemma hm_delete_rule': 
  ...
  unfolding is_hashmap'_def is_hashmap_def
  apply (sep_frame_fwd rule: is_hashtable_addI)
  apply (sep_auto heap: complete_ht_delete)
  using update_ls_delete_map_of
...
lemma hm_delete_rule: 
  ...
  unfolding is_hashmap_def[of m]
  by (sep_auto heap: hm_delete_rule')
...
lemma hm_size_rule': 
  ...
  apply (sep_auto simp: sum_list_length_card_dom_map_of_concat)
  done
...
lemma hm_hashcode_eq:
  ...
  apply (cases "l!i!j")
  apply (force simp: set_conv_nth mod_dist)
  done
...
lemma hm_it_next_rule': "l'\<noteq>[] \<Longrightarrow> 
  ...
  supply [sep_heap_rules] = hm_it_adjust_rule
  apply (cases it;clarsimp split!: nat.splits list.splits)
  subgoal by sep_auto
  subgoal by (sep_auto simp: butlast_append)
  subgoal
    apply (cases "l=[]"; simp?)
    apply (sep_auto simp: )
    by (sep_auto simp: take_Suc_conv_app_nth)
  subgoal by (sep_auto simp: butlast_append)
  done  
...
proof -
  ...
  thus ?thesis
    ...
    subgoal for l l'
      ...
      apply (cases l' rule: rev_cases; sep_auto simp: restrict_map_def aux3)
      done
    done
qed

lemma sep_PRE_is_hashtable[elim!]:
  assumes "sep_PRE (is_hashtable l ht)"  
  obtains "ht_size l (the_size ht)" "ht_hash l" "ht_distinct l" "Suc 0 < length l"
  using assms
  unfolding is_hashtable_def
  by auto

lemma hm_it_init_aux1: "\<lbrakk>ht_distinct l; l\<noteq>[]\<rbrakk> \<Longrightarrow> distinct (map fst (rev (l ! 0)))"
  unfolding ht_distinct_def
  by (metis distinct_rev length_greater_0_conv rev_map)

lemma hm_it_init_aux2: "ht_distinct l \<Longrightarrow> r<length l \<Longrightarrow> map_of (rev (l!r)) = map_of (l!r)"  
  unfolding ht_distinct_def
  by auto
  
(*    
lemma hm_it_init_aux2: "ht_distinct l \<Longrightarrow> l\<noteq>[] \<Longrightarrow> map_of (rev (l!0)) = map_of (l!0)"  
  unfolding ht_distinct_def
  by auto
*)  
  
lemma hm_it_init_aux3: "\<lbrakk>ra \<le> length l - Suc 0; ht_hash l; ht_distinct l\<rbrakk> \<Longrightarrow> distinct (map fst (concat (take ra l)))"  
  by (metis card_distinct distinct_imp_distinct_take dom_map_of_conv_image_fst length_concat length_map
      list.set_map sum_list_length_card_dom_map_of_concat)  
       
lemma hm_it_init_aux4: "\<lbrakk>ra < length l; ht_distinct l\<rbrakk> \<Longrightarrow> distinct (map fst (rev (l ! ra)))"      
  apply (metis distinct_rev ht_distinct_def rev_map)
  done    
       
lemma hm_it_init_aux5: "\<lbrakk>xa \<in> set (take ra l); ra < length l; ht_hash l; ht_distinct l; (aa, ba) \<in> set (l ! ra); (aa, b) \<in> set xa\<rbrakk> 
       \<Longrightarrow> False"  
  apply (smt (verit, ccfv_threshold) ht_hash_def in_set_conv_nth length_take less_irrefl min_less_iff_conj
      nth_take prod.sel(1))
  done    
  
lemma hm_it_init_rule:
  fixes ht :: "('k::{heap,hashable},'v::heap) hashtable"
  shows "<is_hashmap m ht> hm_it_init ht <hm_is_it m ht m>"
  unfolding hm_it_init_def is_hashmap_def is_hashmap'_def 
    hm_is_it_def hm_is_it'_def
  (*supply [simp del] = map_of_append  *)
  supply [simp] = 
    hm_it_init_aux1 hm_it_init_aux2 hm_it_init_aux3 hm_it_init_aux4 
    concat_take_Suc_app_nth
  supply [elim!] = hm_it_init_aux5  
  apply (sep_auto heap: hm_it_adjust_rule)
  done

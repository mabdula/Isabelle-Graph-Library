  ...
  *)
  ...
  next
    ...
    from Suc.prems show ?case
      supply [simp] = take_update_last drop_upd_irrelevant
      by (sep_auto)
  qed
...
subsection \<open>Derived Functions\<close>
  ...
  lemma array_shrink_rule[sep_heap_rules]:
    ...
    shows "< a\<mapsto>\<^sub>ala > array_shrink a s <\<lambda>a'. a'\<mapsto>\<^sub>atake s la >"
  ...
  lemma array_grow_rule[sep_heap_rules]:
    ...
    shows "
      < a\<mapsto>\<^sub>ala > 
        array_grow a s x 
      <\<lambda>a'. a'\<mapsto>\<^sub>a (la @ replicate (s-length la) x)>"
    using assms

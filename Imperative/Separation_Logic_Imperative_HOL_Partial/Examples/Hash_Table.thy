...
lemma is_hashtable_addI: 
  "is_hashtable l ht \<Longrightarrow>\<^sub>A is_hashtable l ht * \<up>(ht_size l (the_size ht)
    \<and> ht_hash l
    \<and> ht_distinct l
    \<and> 1 < length l)"
...
lemma ht_array_nth_rule[sep_heap_rules]:
    "<is_hashtable l ht>
      ...
      <\<lambda>r. is_hashtable l ht * \<up>(r = l!i \<and> i<length l)>"
  unfolding is_hashtable_def
  by sep_auto
...
next
  ...
  proof (cases x)
    case (Pair k v)
    then show ?thesis
      by (sep_auto heap: complete_ht_upd heap: Cons.hyps)
  qed
qed
...
lemma ht_not_empty: "is_hashtable [] ht = false"  
  unfolding is_hashtable_def
  by sep_auto
...
lemma complete_ht_rehash:
  ...
  apply (cases "l=[]"; (simp add: ht_not_empty)?)
  apply (subgoal_tac "Suc 0 < 2 * length l")
  subgoal
    apply (sep_auto heap: complete_ht_new_sz)
    unfolding ht_rehash_def
    by (sep_auto heap: complete_ht_new_sz complete_ht_copy simp: ls_rehash_def)
  subgoal by (cases l; simp)
  done 
...
lemma ht_len_rule: "<is_hashtable l ht> Array.len (the_array ht) <\<lambda>r. is_hashtable l ht * \<up>(r=length l)>"  
  unfolding is_hashtable_def
  by sep_auto
...
lemma complete_ht_update_normal:
  ...
  by (sep_auto heap: ht_len_rule complete_ht_upd)

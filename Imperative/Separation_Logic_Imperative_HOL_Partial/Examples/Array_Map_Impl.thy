...
begin
  ...
  *)
  ...
  lemma iam_new_rule: "<emp> iam_new < is_iam Map.empty >"
    unfolding iam_new_def supply [sep_heap_rules] = iam_new_sz_rule by (sep_auto)
  ...
  lemma iam_update_rule:
    "< is_iam m p > iam_update k v p <\<lambda>r. is_iam (m(k\<mapsto>v)) r>"
    unfolding is_iam_def iam_update_def
    by (sep_auto simp: iam_update_abs1 iam_update_abs2)

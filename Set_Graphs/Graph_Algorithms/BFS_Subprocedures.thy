...
proof(goal_cases)
  ...
  have a: "vset_inv vs \<Longrightarrow>
       ...
       b \<in> [case if a = u then Some (x2 \<union>\<^sub>G vs) else lookup G a of None \<Rightarrow> \<emptyset>\<^sub>V | Some vset \<Rightarrow> vset]\<^sub>s \<Longrightarrow>
  ...
  have c: "b \<in> [case if a = u then Some vs else lookup G a of None \<Rightarrow> \<emptyset>\<^sub>V | Some vset \<Rightarrow> vset]\<^sub>s \<Longrightarrow>
  ...
  have d: "\<forall>v vset. lookup G v = Some vset \<longrightarrow> vset_inv vset \<Longrightarrow>
           b \<in> [case if a = u then Some vs else lookup G a of None \<Rightarrow> \<emptyset>\<^sub>V | Some vset \<Rightarrow> vset]\<^sub>s \<Longrightarrow>
  ...
  show ?case 
    ...
    by(auto split: if_split option.split 
            simp add:  add_neighbs_def Graph.digraph_abs_def 
                      Graph.neighb_def Graph.adjmap.map_update Graph.graph_inv_def
...
"next_frontier frontier vis= 
   fold_vset (\<lambda> u nf. nf \<union>\<^sub>G ((\<N>\<^sub>G u) -\<^sub>G vis)) frontier \<emptyset>\<^sub>V"
...
proof-
  ...
  have goal1_gen:"vset_inv (foldr (\<lambda>u nf. nf \<union>\<^sub>G (\<N>\<^sub>G u -\<^sub>G vis)) xs \<emptyset>\<^sub>V)" for xs
   using assms 
  ...
  moreover have "foldr (\<lambda>u nf. nf \<union> [(\<N>\<^sub>G u -\<^sub>G vis)]\<^sub>s) xs {} = [(foldr (\<lambda>u nf. nf \<union>\<^sub>G (\<N>\<^sub>G u -\<^sub>G vis)) xs \<emptyset>\<^sub>V)]\<^sub>s"  for xs
    by (induction xs)
       (simp add: assms(2) assms(3) goal1_gen)+
  ultimately have "[(foldr (\<lambda>u nf. nf \<union>\<^sub>G (\<N>\<^sub>G u -\<^sub>G vis)) xs \<emptyset>\<^sub>V)]\<^sub>s =
                      [(foldr (\<lambda>u nf. nf \<union>\<^sub>G (\<N>\<^sub>G u -\<^sub>G vis)) (remdups xs) \<emptyset>\<^sub>V)]\<^sub>s" for xs
    by auto
  moreover obtain xs where xs_props:
       "set xs = [frontier]\<^sub>s"
       "[next_frontier frontier vis]\<^sub>s =
          [foldr (\<lambda>u nf. nf \<union>\<^sub>G (\<N>\<^sub>G u -\<^sub>G vis)) xs \<emptyset>\<^sub>V]\<^sub>s"
  ...
  ultimately obtain xs' where xs'_props: 
    ...
    by (metis )
  moreover have "[foldr (\<lambda>u nf. nf \<union>\<^sub>G (\<N>\<^sub>G u -\<^sub>G vis)) xs' \<emptyset>\<^sub>V]\<^sub>s =
  ...
  have "vset_inv (foldr (\<lambda>u nf. nf \<union>\<^sub>G (\<N>\<^sub>G u -\<^sub>G vis)) xs \<emptyset>\<^sub>V)" for xs
   using assms 
...
"expand_tree BFS_tree frontier vis = 
   fold_adjmap 
     (\<lambda> u nf. add_neighbs nf u (diff (\<N>\<^sub>G u) vis)) frontier BFS_tree"
...
proof-
  ...
  obtain xs where xs_prop:
    "set xs = [frontier]\<^sub>s" "expand_tree BFS_tree frontier vis = foldr (\<lambda> u nf. add_neighbs nf u (diff (\<N>\<^sub>G u) vis)) xs BFS_tree"
    using fold_adjmap[OF assms(2), of _ BFS_tree]
    by (auto simp add: expand_tree_def)
  have goal1_general:"Graph.graph_inv (foldr  (\<lambda> u nf. add_neighbs nf u (diff (\<N>\<^sub>G u) vis))
                          xs BFS_tree)" for xs
  proof(induction xs)
    case Nil
  ...
  have one_it_graph_abs:"Graph.graph_inv T \<Longrightarrow> 
                 [(\<lambda> u nf. add_neighbs nf u (diff (\<N>\<^sub>G u) vis)) u T]\<^sub>G 
  ...
  have "[foldr (\<lambda> u nf. add_neighbs nf u (diff (\<N>\<^sub>G u) vis)) xs BFS_tree]\<^sub>G = [BFS_tree]\<^sub>G \<union> {(u, v) |u v. u \<in> [frontier]\<^sub>s \<and> v \<in> neighbourhood [G]\<^sub>G u - [vis]\<^sub>s}"
  ...
  next
    ...
    proof(cases "a \<in> set xs")
      case True
      have heper: "[foldr (\<lambda> u nf. add_neighbs nf u (diff (\<N>\<^sub>G u) vis)) xs BFS_tree]\<^sub>G \<union> {(a, v) |v. v \<in> neighbourhood [G]\<^sub>G a - [vis]\<^sub>s} =

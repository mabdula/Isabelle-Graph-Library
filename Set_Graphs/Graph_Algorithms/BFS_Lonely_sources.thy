...
definition "BFS_axiom \<longleftrightarrow>
  ...
  t_set srcs \<noteq> {} \<and> vset_inv srcs" (* \<and>
  t_set srcs \<subseteq> dVs (Graph.digraph_abs G)"*)
...
next
 ...
 \<subseteq> dVs (Graph.digraph_abs G) \<union> t_set srcs"
  ...
  ultimately have **: "dVs (Graph.digraph_abs G) \<union> t_set srcs - (t_set (visited (BFS_upd1 BFS_state)) \<union>
                                                      t_set (current (BFS_upd1 BFS_state)))\<noteq>
                          dVs (Graph.digraph_abs G) \<union> t_set srcs - (t_set (visited BFS_state) \<union> t_set (current BFS_state))"
    using \<open>BFS_call_1_conds BFS_state\<close> \<open>invar_1 BFS_state\<close> \<open>invar_2 BFS_state\<close> 
    by(auto elim!: invar_props_elims call_cond_elims
          simp add: BFS_upd1_def Let_def dVs_def
          intro!: mlex_less psubset_card_mono)
...
proof-
  ...
  have *: "distance_set (Graph.digraph_abs G) (t_set srcs) v = 0" if "v \<in> t_set srcs" 
            "v \<in> dVs (Graph.digraph_abs G)"for v

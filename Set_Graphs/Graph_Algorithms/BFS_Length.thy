...
record ('parents, 'vset, 'pmap) BFS_combined_state = 
    parents:: "'parents" current:: "'vset" visited:: "'vset" pred::"'pmap"


locale BFS_with_length =
BFS where  lookup = lookup   and srcs = srcs and G = G
+
pred_map: Map pred_empty pred_update pred_delete pred_lookup pred_inv
for lookup :: "'adjmap \<Rightarrow> 'ver \<Rightarrow> 'vset option" and srcs and G::"'adjmap"
and
pred_empty pred_update pred_delete and pred_lookup:: "'pmap \<Rightarrow> 'ver \<Rightarrow> 'ver option"
and  pred_inv+
fixes expand_pred::"'pmap \<Rightarrow> 'vset \<Rightarrow> 'vset \<Rightarrow> 'pmap"
assumes
   expand_pred_map[simp]:
     "\<lbrakk>pred_inv pmap; vset_inv frontier; vset_inv vis; Graph.graph_inv G\<rbrakk> \<Longrightarrow> 
       pred_inv (expand_pred pmap frontier vis)"
     "\<lbrakk>pred_inv pmap; vset_inv frontier; vset_inv vis; Graph.graph_inv G\<rbrakk>
      \<Longrightarrow> (\<And> v.  \<nexists> u. u \<in> t_set (frontier)\<and> 
                       v \<in> (Pair_Graph.neighbourhood (Graph.digraph_abs G) u -
                       t_set vis)
       \<Longrightarrow> pred_lookup (expand_pred pmap frontier vis) v = pred_lookup pmap v)"
     "\<lbrakk>pred_inv pmap; vset_inv frontier; vset_inv vis; Graph.graph_inv G\<rbrakk>
      \<Longrightarrow> (\<And> v u.  u \<in> t_set (frontier) \<Longrightarrow> 
                       v \<in> (Pair_Graph.neighbourhood (Graph.digraph_abs G) u -
                       t_set vis)
       \<Longrightarrow> (\<exists> u'. pred_lookup (expand_pred pmap frontier vis) v = Some u'
                  \<and> u' \<in> t_set (frontier) \<and> 
                   v \<in> (Pair_Graph.neighbourhood (Graph.digraph_abs G) u' -
                       t_set vis)))"



     (*"\<lbrakk>pred_inv pmap; vset_inv frontier; vset_inv vis; Graph.graph_inv G;
       t_set frontier \<subseteq> t_set vis; dom (pred_lookup pmap) \<inter> t_set (next_frontier frontier vis) = {}\<rbrakk> \<Longrightarrow>
         \<exists> X. { (u, v) | u v. pred_lookup (expand_pred pmap frontier vis) v = Some u} = 
         { (u, v) | u v. pred_lookup pmap v = Some u} \<union> X \<and> 
         (\<forall> u v. (u, v) \<in> X \<longrightarrow> u \<in> t_set frontier \<and> v \<in> t_set (next_frontier frontier vis)
                 \<and> (u, v) \<in> Graph.digraph_abs G) \<and>
         (\<forall> v. v \<in> t_set (next_frontier frontier vis)\<longrightarrow> (\<exists> u.  u \<in> t_set frontier \<and> (u, v) \<in> X))"  *)
begin


function (domintros) BFS_combined::"('adjmap, 'vset, 'pmap) BFS_combined_state 
                                  \<Rightarrow> ('adjmap, 'vset, 'pmap) BFS_combined_state" where
  "BFS_combined BFS_state = 
     (
        if current BFS_state \<noteq> \<emptyset>\<^sub>N then
          let
            ...
            pred'    = expand_pred (pred BFS_state) (current BFS_state) visited'
          in 
            BFS_combined (BFS_state \<lparr>parents:= parents', visited := visited', 
                                     current := current', pred:=pred'\<rparr>)
         else
          BFS_state)"
  by pat_completeness auto

partial_function (tailrec) BFS_combined_impl::"('adjmap, 'vset, 'pmap) BFS_combined_state 
                                  \<Rightarrow> ('adjmap, 'vset, 'pmap) BFS_combined_state" where
  "BFS_combined_impl BFS_state = 
     (
        if current BFS_state \<noteq> \<emptyset>\<^sub>N then
          let
            visited' = visited BFS_state \<union>\<^sub>G current BFS_state;
            ...
            pred'    = expand_pred (pred BFS_state) (current BFS_state) visited'
          in 
            BFS_combined_impl (BFS_state \<lparr>parents:= parents', visited := visited', 
                                     current := current', pred:=pred'\<rparr>)
         else
          BFS_state)"

abbreviation "remove_pred state ==
           \<lparr> BFS_state.parents = BFS_combined_state.parents state ,
             BFS_state.current = BFS_combined_state.current state, 
             BFS_state.visited = BFS_combined_state.visited state \<rparr>"

lemma BFS_impl_combined_same:
  assumes "BFS_combined_dom state" 
  shows "BFS_combined_impl state = BFS_combined state"
  by(induction rule: BFS_combined.pinduct[OF assms])(auto simp add: BFS_combined_impl.simps BFS_combined.psimps)

lemma BFS_combined_dom_from_usual:
  assumes "BFS_dom state'" "(remove_pred state) = state'"
  shows "BFS_combined_dom state"
  using assms
  by(induction arbitrary: state rule: BFS.pinduct)(auto intro: BFS_combined.domintros)

lemma combined_to_BFS_dom:
  assumes "BFS_combined_dom state" "(remove_pred state) = state'"
  shows "BFS_dom state'"
  using assms
  by(induction arbitrary: state' rule: BFS_combined.pinduct)(auto intro: BFS.domintros)

lemma same_dom: "BFS_dom (remove_pred state) \<longleftrightarrow> BFS_combined_dom state"
  using BFS_combined_dom_from_usual combined_to_BFS_dom by blast

definition "BFS_combined_upd1 BFS_state =
 ( let
            visited' = visited BFS_state \<union>\<^sub>G current BFS_state;
            parents' = expand_tree (parents BFS_state) (current BFS_state) visited';
            current' = next_frontier (current BFS_state) visited';
            pred'    = expand_pred (pred BFS_state) (current BFS_state) visited'
          in BFS_state \<lparr>parents:= parents', visited := visited', 
                                     current := current', pred:=pred'\<rparr>)"

definition "combined_initial_state = \<lparr>parents =  empty, current = srcs, visited = \<emptyset>\<^sub>N, pred = pred_empty\<rparr>"

definition "pred_graph pmap = { (u, v) | u v. pred_lookup pmap v = Some u}"

definition "invar_pred_in_parents state = (\<forall> v u. pred_lookup (pred state) v = Some u \<longrightarrow>
                                           (u, v) \<in> Graph.digraph_abs (parents state))"
...
lemma
  assumes "pred_inv (pred state)"
          "invar_pred_in_parents state"
          "invar_1 (remove_pred state)"
          "invar_3_1 (remove_pred state)"
          "invar_3_2 (remove_pred state)"
          "invar_3_3 (remove_pred state)"
          "invar_3_4 (remove_pred state)"
          "invar_goes_through_current (remove_pred state)"
          "invar_dist' (remove_pred state)"
          "invar_parents_shortest_paths (remove_pred state)"
  shows   "invar_pred_in_parents (BFS_combined_upd1 state)"
proof-
  define visited' where "visited' = visited state \<union>\<^sub>G current state"
  define parents' where "parents' = expand_tree (parents state) (current state) visited'"
  define current' where "current' = next_frontier (current state) visited'"
  define pred' where "pred'    = expand_pred (pred state) (current state) visited'"
  have pred'_is:"pred (BFS_combined_upd1 state) = pred'" 
    by(simp add:  pred'_def visited'_def BFS_combined_upd1_def Let_def)
  have parents'_is:"parents (BFS_combined_upd1 state) = parents'" 
    by(simp add:  parents'_def visited'_def BFS_combined_upd1_def Let_def)
  note invars_unfolded = assms(3-)[simplified invar_1_def invar_3_1_def invar_3_2_def invar_3_3_def invar_3_4_def
                           invar_goes_through_current_def invar_dist'_def invar_parents_shortest_paths_def,
               simplified]
  have G_invar: "Graph.graph_inv G" 
    by (simp add: \<open>BFS_axiom\<close>)
  have  inv_visited: "vset_inv (BFS_combined_state.visited state)"
    by (simp add: invars_unfolded(1))
  have  inv_current: "vset_inv (BFS_combined_state.current state)"
    by (simp add: invars_unfolded(1))
  have inv_parents: "Graph.graph_inv (parents state)"
    by (simp add: invars_unfolded(1))  


  show ?thesis
    unfolding invar_pred_in_parents_def pred'_is parents'_is


    thm expand_pred_map(2,3)[OF assms(1) inv_current inv_visited G_invar
          ] sym[OF pred'_def[simplified visited'_def]]
  thm expand_tree[OF inv_parents inv_current inv_visited G_invar]



  thm assms(3-)[simplified invar_1_def invar_3_1_def invar_3_2_def invar_3_3_def invar_3_4_def
                           invar_goes_through_current_def invar_dist'_def invar_parents_shortest_paths_def,
               simplified]
